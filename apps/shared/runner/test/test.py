...
import grpc
import typer
from protos.mtib_runner.mtib_runner_pb2 import (AccelReadMaxRequest,
                                                AccelReadMaxResponse,
                                                AccelReadRequest,
                                                AccelReadResponse, AdcChannel,
                                                AdcReadAllRequest,
                                                AdcReadAllResponse,
                                                AdcReadRequest,
                                                AdcReadResponse,
...
SERVER_ADDRESSES = ["localhost:50053"]
...
def check_step_4(server: MtibRunnerStub) -> bool:
    ...
    if not uvp_n_success:
        ...
        return False
    elif uvp_n_value is not True:  # Assuming digital high is >= 0.8V
    ...
    if not EXPECTED_CURRENT_MIN_MA <= current_value <= EXPECTED_CURRENT_MAX_MA:
        logging.error(
            f"Step 4.f failed: Expected current consumption within normal range, Actual current = {current_value}A"
        )
        return False
    else:
...
def check_step_6(server: MtibRunnerStub) -> bool:
    ...
    if not vin_success or not vbat_success:
        ...
        return False
    elif abs(vin_value - vbat_value) > 0.125:
...
def flash_device(
    server, server_address: str, firmware_file: str, device: DeviceType, is_modem_fw: Optional[bool]
) -> bool:
