    ...
    def initialize_and_find_jlinks(self):
        ...
        if len(initial_scan) == 1:
            ...
            if new_devices:
                ...
                raise ValueError("No new J-Link device found after enabling the second device.")
        else:
            raise ValueError(f"Initial scan did not find exactly one J-Link device, found: {len(initial_scan)}.")
