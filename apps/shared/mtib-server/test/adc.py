...
def sample(client: MtibV1Client, logger: Logger) -> None:
    ...
    for i in range(8):
        ...
        end_time = time.time()
        logger.info(f"Time taken to read ADC {i}: {(end_time - start_time) * 1000}ms")
    ...
    if error:
        logger.error(f"Error reading all ADCs: {error}")
        sys.exit(1)
    end_time = time.time()
    logger.info(f"Time taken to read all ADCs: {(end_time - start_time) * 1000}ms")
