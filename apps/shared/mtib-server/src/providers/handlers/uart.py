...
class UartHandler:
    def __init__(self, logger: Logger):
        ...
        self.uart_configs = {
            HostType.HOST_TYPE_NRF9160: {
                ...
                'timeout': 1,
                'write_timeout': 5,  # Increased write timeout
                'xonxoff': False,
                'rtscts': False,
                'dsrdtr': False
            },
        ...
        try:
            ...
            if target in self.rx_threads and target in self.tx_threads:
                status['threads_running'] = (
                    self.rx_threads[target].is_alive() and 
                    self.tx_threads[target].is_alive()
                )
    ...
    def _ensure_connection(self, target: HostType) -> Optional[str]:
        ...
        with self.connection_locks[target]:
            ...
            try:
                # Try to reset UART state first using stty
                try:
                    import subprocess
                    subprocess.run(['stty', '-F', device_path, 'sane'], 
                                 capture_output=True, timeout=5)
                    subprocess.run(['stty', '-F', device_path, str(config['baudrate'])], 
                                 capture_output=True, timeout=5)
                    self.logger.info(f"Reset UART state for {device_path}")
                except Exception as e:
                    self.logger.warning(f"Could not reset UART state for {device_path}: {e}")
                ...
                try:
                    ...
                    if data and not self.rx_stop_events[target].is_set():
                        ...
                        if bytes_written != len(data):
                            self.logger.error(f"Failed to write all data to {target}: wrote {bytes_written}/{len(data)} bytes")
        ...
        try:
            ...
            for request in request_iterator:
                ...
                if request.data:
                    try:
                        ...
                        self.logger.debug(f"Queued {len(request.data)} bytes for {current_target}: {request.data.hex()}")
                    except Exception as e:
            ...
            if current_target:
                ...
                if not self.client_streams[current_target]:
                    ...
                    self.logger.info(f"Client disconnected from {current_target}, {len(self.client_streams[current_target])} clients remaining")
