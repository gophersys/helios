...
class GpioHandler:
    def __init__(self, gpios: list[Gpio], logger: Logger):
        self.gpios = gpios
        self.logger = logger
    ...
    def config(self, request: GpioConfigRequest, context: grpc.ServicerContext) -> GpioConfigResponse:
        ...
        if request.gpio >= len(self.gpios):
            return GpioConfigResponse(
                success=False,
                message=f"Invalid GPIO number {request.gpio}. Must be between 0 and {len(self.gpios)-1}"
            )
