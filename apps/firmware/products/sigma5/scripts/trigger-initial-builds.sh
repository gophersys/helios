#!/bin/bash
# Trigger initial Sigma5 firmware builds via Concord API
...
#   - sigma5_fw: release + debug variants (nrf52840 + nrf9160)
#   - sigma5_mfg_fw: mfg variant
...
# Get latest commit SHA from sigma5_fw
echo -e "${CYAN}Fetching latest commit SHA from sigma5_fw...${NC}"
COMMIT_SHA=$(git ls-remote git@bitbucket.org:corekinect/sigma5_fw.git refs/heads/concord-main | cut -f1)
...
PIPELINES=(
    # Production firmware pipeline (sigma5_fw)
    "{\"product\":\"sigma5_fw\",\"board\":\"sigma5_b0\",\"branch\":\"concord-main\",\"commitSha\":\"$COMMIT_SHA\",\"triggerType\":\"manual\",\"buildVariant\":\"debug\",\"name\":\"Sigma5 FW Initial Build\"}"
    # Manufacturing firmware pipeline (sigma5_mfg_fw)
    "{\"product\":\"sigma5_mfg_fw\",\"board\":\"sigma5_c0\",\"branch\":\"concord-main\",\"triggerType\":\"manual\",\"name\":\"Sigma5 MFG FW Initial Build\"}"
)

echo -e "\n${CYAN}Triggering initial Sigma5 pipelines...${NC}"
