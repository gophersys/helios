...
class Pin(Enum):
    """
    Enum for the Toradex Verdin iMX8MM SoM pins exposed in the RAM SODIMM connector.
    """
...
class Gpio:
    ...
    def write(self, value: int) -> Optional[str]:
        ...
        try:
            ...
            # Track the state for output pins
    ...
    def read(self) -> tuple[Optional[str], int]:
        ...
        # For output pins, return the tracked state
        if self.direction == Direction.OUTPUT:
            return None, self._state

        # For input pins, read the actual pin value
        try:
            value = self.request.get_value(self.pin.value)
            actual_value = 1 if value == Value.ACTIVE else 0
            # Update tracked state for input pins too
            self._state = actual_value
