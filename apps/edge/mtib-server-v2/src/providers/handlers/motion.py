...
class MotionHandler:
    """Handler for motion control via FluidNC.

    On REV 1.2, this handler can control motor power via the TCA9534A GPIO expander.
    ...
    def __init__(
        ...
        # Log motor power control capability
        if hardware and hardware.has_motor_power_switch:
            self.logger.info("Motor power control available (REV 1.2)")
        else:
            self.logger.info("Motor power always on (REV 1.1 or hardware not available)")
    ...
    def _enable_motor_power(self, enable: bool) -> Optional[str]:
        """Enable or disable motor power (REV 1.2 only).

        On REV 1.1, this is a no-op since motor power is always on.

        Returns:
            None on success, error message on failure
        """
        if self.hardware is None:
            return None  # No hardware context, assume power is managed externally

        try:
            self.hardware.set_motor_power(enable)
            return None
