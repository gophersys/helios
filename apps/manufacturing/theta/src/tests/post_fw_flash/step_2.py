...
def fw_flash_test_step_2_handler(config: Sigma5ManufacturingConfig, node: str, usr_data: None) -> TestStepResult:
    ...
    result.error = runnners_controller.flash_fw_file(
        node, config.post_fw_flash_test_nrf9160_app_fw_name, DeviceType.DEVICE_NRF9160, False
    ...
    # Flash 52840
    result.error = runnners_controller.flash_fw_file(
        node, config.post_fw_flash_test_nrf52840_app_fw_name, DeviceType.DEVICE_NRF82840, False
    )  # It is NOT modem firmware
