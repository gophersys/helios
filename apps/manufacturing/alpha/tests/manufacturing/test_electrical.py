...
def test_electrical(slot, config, report):
    ...
    with report.step("Apply +3.4V to +BATT_IN"):
        err = mtib.PowerEnable(channel=0, voltage_v=uvlo_voltage)
    ...
    with report.step("Apply +3.7V to +BATT_IN"):
        err = mtib.PowerEnable(channel=0, voltage_v=nominal_voltage)
    ...
    with report.step("Apply +4.5V to +BATT_IN"):
        err = mtib.PowerEnable(channel=0, voltage_v=high_voltage)
