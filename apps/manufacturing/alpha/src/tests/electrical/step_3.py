...
def electrical_test_step_3_handler(
    ...
    """
    Test plan step 3: Apply +3.7V to +BATT_IN
    """
    result: TestStepResult = TestStepResult(success=False)

    result.error = mtib_servers.enable_power(node, config.electrical_nominal_voltage_v)
    if result.error:
        return result

    logging.debug(f"Step 3: Applied {config.electrical_nominal_voltage_v}V to +BATT_IN")
...
electrical_test_step_3: TestStep = TestStep(
    info=StepInfo(
        name="Apply +3.7V to +BATT_IN",
        description="Applies 3.7V (above UVLO threshold) to the battery input.",
        noPassIsFatal=True,
    ),
    timeout_ms=60000,
