...
class Test:
    ...
    #                                                                          Test
    #  --------------------------------------------------------------------------*/
    def run(self, nodes: List[str], config: str = None) -> str:
        # Initialize
        error = self.init(config, nodes)
        if error:
        ...
        with self.lock:
            ...
            if self.config_type is not None:
                if not config or config.strip() == "{}":
                    ...
                    try:
                        # Parse the provided config and merge non-default fields
                        provided_data = json.loads(config)
                        self.config_obj = self.config_type()

                        for key, value in provided_data.items():
                            if hasattr(self.config_obj, key):
                                setattr(self.config_obj, key, value)
                                logging.debug(f"Config override: {key} = {value}")

                        logging.debug(f"Using default configuration with provided overrides")
                    except Exception as e:
