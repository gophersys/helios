...
def _verify_comms_flash(node: str, config: ThetaFixtureConfig) -> Optional[str]:
    ...
    def write_func(addr, data):
        return mtib_servers.theta_cmd_write_ext_flash(node, addr, data)

    def read_func(addr, length):
        return mtib_servers.theta_cmd_read_ext_flash(node, addr, length)
    ...
    if not err:
        # Cleanup erase
        mtib_servers.theta_cmd_erase_ext_flash(node)
        return None

    # Erase and retry
    logging.debug(f"[Comms] First attempt failed: {err}. Erasing and retrying...")
    erase_success, erase_err = mtib_servers.theta_cmd_erase_ext_flash(node)
...
def _verify_app_flash(node: str, config: ThetaFixtureConfig) -> Optional[str]:
    ...
    def write_func(addr, data):
        return mtib_servers.theta_app_cmd_write_ext_flash(node, addr, data)

    def read_func(addr, length):
        return mtib_servers.theta_app_cmd_read_ext_flash(node, addr, length)
    ...
    if not err:
        # Cleanup erase
        mtib_servers.theta_app_cmd_erase_ext_flash(node)
        return None

    # Erase and retry
    logging.debug(f"[App] First attempt failed: {err}. Erasing and retrying...")
    erase_success, erase_err = mtib_servers.theta_app_cmd_erase_ext_flash(node)
    if not erase_success or erase_err:
...
def post_step_8_handler(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        futures = {
            executor.submit(_verify_comms_flash, node, config): "comms",
            executor.submit(_verify_app_flash, node, config): "app",
        }
