...
class ThetaMtibServers:
    ...
    def theta_cmd_get_imei_iccid(self, host: str) -> Tuple[Optional[str], Optional[List[str]], Optional[str]]:
        ...
        response, error = self._send_uart_command(host, "get_imei_iccid", ["IMEI:"], timeout=30)
        if error:
        ...
        # Parse ICCIDs (may be multiple lines)
        idx = 0
        while True:
    ...
    def theta_cmd_lock_shell(self, host: str) -> Tuple[bool, Optional[str]]:
        ...
        response, error = self._send_uart_command(host, "lock_shell", ["Locking shell mode ON"])
        if error:
            return False, error
        return True, None
    ...
    def theta_cmd_debug_uart_disable(self, host: str) -> Tuple[bool, Optional[str]]:
        ...
        response, error = self._send_uart_command(host, "debug_enable 0", ["Debug is not enabled"])
        if error:
            return False, error
        return True, None
    ...
    def theta_cmd_personalize(self, host: str, device_id: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        response, error = self._send_uart_command(
            host, f"personalize {device_id}", ["Personalization complete"], timeout=15
        )
        if error:
            return None, None, error
        ...
        hex_start = response.find("Public key (hex):")
        if hex_start != -1:
            hex_line_end = response.find("\n", hex_start)
            if hex_line_end == -1:
                hex_line_end = len(response)
            hex_key = response[hex_start:hex_line_end].split(":", 1)[1].strip()

        # Parse base64 public key
        b64_start = response.find("Public key (base64):")
        if b64_start != -1:
            b64_line_end = response.find("\n", b64_start)
            if b64_line_end == -1:
                b64_line_end = len(response)
            base64_key = response[b64_start:b64_line_end].split(":", 1)[1].strip()
    ...
    def theta_app_cmd_lock_shell(self, host: str) -> Tuple[bool, Optional[str]]:
        ...
        response, error = self._send_app_uart_command(
            host, "lock_shell", ["Locking shell mode ON", "Shell locked"], timeout=30
        )
        if error:
            return False, error
        return True, None
