...
<style>
  /* Design spike (spike/shadcn-theme): the card is a shadcn Card — a --card surface, a 1px --border
     hairline, --radius-lg geometry, and the refined --shadow ramp (resting xs → hover md). All tokens
     derive from @eden/theme (via +layout.svelte's SHADCN SKIN MAPPING). */
  .card {
    display: flex;
