...
<style>
  ...
  /* Design spike (spike/shadcn-theme): the rail links read as shadcn sidebar rows — --radius-sm
     corners, a --muted-foreground rest state, a --muted hover fill, and a solid --accent-surface
     selected state with a full-ink label. */
  .nav__link {
    display: flex;
    align-items: center;
  ...
  .nav__avatar {
    ...
    block-size: 32px;
    border-radius: 999px;
    background: color-mix(in oklab, var(--primary, var(--eden-app-accent)) 22%, var(--card, var(--eden-app-panel-bg)));
    color: var(--foreground, var(--eden-app-fg));
    font-size: 0.78rem;
    font-weight: 600;
    flex-shrink: 0;
