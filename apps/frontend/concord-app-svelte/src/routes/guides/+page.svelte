    ...
    {#each guides as guide}
      ...
      <div class="card">
        <button
          ...
          class="flex w-full items-center gap-3 px-5 py-4 text-left hover:bg-surface-2 rounded-xl transition-colors"
        >
          {#if isExpanded}
          ...
          {/if}
          <Icon size={18} strokeWidth={1.75} class="shrink-0 text-accent" />
          ...
          <div class="border-t border-border px-5 py-4">
            <ol class="space-y-4">
              {#each guide.steps as step, i}
                <li class="flex gap-3">
                  <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-accent-muted text-2xs font-semibold text-accent">
                    {i + 1}
                  </span>
                  <div>
