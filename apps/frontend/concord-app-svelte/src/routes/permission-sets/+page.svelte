<script lang="ts">
  ...
  onMount(() => {
    if (!auth.hasPermission('Concord.Admin.PermissionSets.View')) {
      goto('/');
      return;
    }
    fetchSets();
...
{#snippet PermissionSetCard(ps: PermissionSet)}
  ...
  <div class="card">
    <!-- Header row - use div with role for toggle, separate buttons for actions -->
...
<svelte:head>
  <title>Permission Sets — Concord</title>
</svelte:head>
