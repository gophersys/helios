<script lang="ts">
  ...
  async function fetchDeployments() {
    try {
      const url = namespace ? `/v2/system/deployments?namespace=${namespace}` : '/v2/system/deployments';
        ...
        <tbody>
          {#each deployments as deploy}
            <tr
              ...
              onclick={() => goto(`/system/deployments/${deploy.namespace}/${deploy.name}`)}
              role="button"
              tabindex="0"
              onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && goto(`/system/deployments/${deploy.namespace}/${deploy.name}`)}
            >
