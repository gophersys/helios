<script lang="ts">
  ...
  async function fetchDeployment() {
    loading = true;
    try {
      const res = await api.get<{ data: DeploymentDetailData }>(`/v2/system/deployments/${namespace}/${deploymentName}`);
      if (res?.data) {
        deployment = res.data;
  ...
  async function scaleDeployment() {
    ...
    mutationError = null;
    try {
      await api.post(`/v2/system/deployments/${namespace}/${deploymentName}/scale`, { replicas: scaleValue });
  ...
  async function restartDeployment() {
    ...
    mutationError = null;
    try {
      await api.post(`/v2/system/deployments/${namespace}/${deploymentName}/restart`);
      await fetchDeployment();
    } catch (e) {
...
<div class="space-y-6">
  <!-- Back Button -->
  <button
    onclick={() => goto('/system/deployments')}
  ...
  {#if loading}
    <div class="flex items-center justify-center py-12">
      <span class="text-secondary">Loading deployment details...</span>
    </div>
  {:else if error}
          ...
          <tbody>
            {#each deployment.pods as pod}
              <tr
                ...
                onclick={() => goto(`/system/pods/${pod.namespace}/${pod.name}`)}
                role="button"
                tabindex="0"
                onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && goto(`/system/pods/${pod.namespace}/${pod.name}`)}
              >
