<script lang="ts">
  ...
  async function fetchServices() {
    try {
      const url = namespace ? `/v2/system/services?namespace=${namespace}` : '/v2/system/services';
        ...
        <tbody>
          {#each services as svc}
            <tr
              ...
              onclick={() => goto(`/system/services/${svc.namespace}/${svc.name}`)}
              role="button"
              tabindex="0"
              onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && goto(`/system/services/${svc.namespace}/${svc.name}`)}
            >
