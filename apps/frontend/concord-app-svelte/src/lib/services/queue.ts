...
export async function getQueueEntry(entryId: string): Promise<ValidationQueueEntry> {
  const res = await apiFetch<ApiResponse<ValidationQueueEntry>>(`/v2/validation/queue/${entryId}`);
  return res.data;
}
...
export async function createQueueEntry(data: {
  ...
  const res = await apiFetch<ApiResponse<ValidationQueueEntry>>('/v2/validation/queue', {
    method: 'POST',
...
export async function cancelQueueEntry(entryId: string): Promise<ValidationQueueEntry> {
  const res = await apiFetch<ApiResponse<ValidationQueueEntry>>(`/v2/validation/queue/${entryId}/cancel`, {
    method: 'POST',
  });
  return res.data;
}
...
export async function promoteQueueEntry(entryId: string): Promise<ValidationQueueEntry> {
  const res = await apiFetch<ApiResponse<ValidationQueueEntry>>(`/v2/validation/queue/${entryId}/promote`, {
    method: 'POST',
  });
  return res.data;
}
...
export async function getQueueStats(): Promise<QueueStats> {
  const res = await apiFetch<ApiResponse<QueueStats>>('/v2/validation/queue/stats');
  return res.data;
}
