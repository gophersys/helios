<script lang="ts">
  ...
  async function fetchDeployments() {
    try {
      const res = await apiFetch<ApiResponse<{ data: ConcordDeployment[] }>>('/v2/kubernetes/managed-deployments');
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      await api.post('/v2/kubernetes/managed-deployments', body);
      resetForm();
      fetchDeployments();
    } catch (err) {
  ...
  async function handleDeploy(id: string) {
    error = null;
    try {
      await api.post(`/v2/kubernetes/managed-deployments/${id}/deploy`);
      fetchDeployments();
    } catch (err) {
  ...
  async function handleStop(id: string) {
    error = null;
    try {
      await api.post(`/v2/kubernetes/managed-deployments/${id}/stop`);
      fetchDeployments();
    } catch (err) {
  ...
  async function handleRestart(id: string) {
    error = null;
    try {
      await api.post(`/v2/kubernetes/managed-deployments/${id}/restart`);
      fetchDeployments();
    } catch (err) {
  ...
  async function handleDelete(id: string) {
    error = null;
    try {
      await api.delete(`/v2/kubernetes/managed-deployments/${id}`);
      fetchDeployments();
    } catch (err) {
