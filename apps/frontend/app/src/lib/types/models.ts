...
export interface ManufacturingPassCriteria {
  ...
  timingLimits: Record<string, number>;
}
...
export type ManufacturingSessionStatus = 'ACTIVE' | 'COMPLETED' | 'CANCELLED';
export type ManufacturingUnitStatus = 'QUEUED' | 'RUNNING' | 'PASSED' | 'FAILED' | 'SKIPPED';
export type ManufacturingStageType = 'ELECTRICAL' | 'FLASH' | 'POST';
...
export interface ManufacturingSession {
  ...
  productId: string;
  fixtureId: string;
  status: ManufacturingSessionStatus;
  ...
  fixture?: { id: string; name: string; slotCount: number } | null;
...
export interface ManufacturingUnit {
  ...
  status: ManufacturingUnitStatus;
...
export interface ManufacturingSessionDetail extends ManufacturingSession {
  activePanel: ManufacturingPanel | null;
