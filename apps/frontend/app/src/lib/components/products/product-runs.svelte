...
<div>
  <div class="mb-3 flex items-center justify-between">
    <h3 class="text-sm font-semibold text-(--color-text-primary)">Validation Runs</h3>
    ...
    <div class="rounded-lg border border-border bg-surface-0">
      {#each Array(3) as _, i}
        <div class="flex items-center gap-4 px-3 py-3" class:border-t={i > 0} class:border-[var(--color-border)]={i > 0}>
          <div class="h-5 w-16 animate-pulse rounded bg-surface-2"></div>
          <div class="h-4 w-40 animate-pulse rounded bg-surface-2"></div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-border bg-surface-1">
            <th class="px-3 py-2 text-left text-xs font-medium text-(--color-text-tertiary)">Name</th>
            <th class="px-3 py-2 text-left text-xs font-medium text-(--color-text-tertiary)">Status</th>
            <th class="px-3 py-2 text-left text-xs font-medium text-(--color-text-tertiary)">Results</th>
            <th class="px-3 py-2 text-right text-xs font-medium text-(--color-text-tertiary)">Duration</th>
          </tr>
        </thead>
        <tbody>
          {#each runs as run (run.id)}
            <tr
              class="border-b border-border last:border-0 hover:bg-surface-1 transition-colors cursor-pointer"
              onclick={() => goto(`/validation/runs/${run.id}`)}
            >
              <td class="px-3 py-2">
                <span class="text-sm font-medium text-(--color-text-primary) truncate block max-w-[200px]">
                  {run.name}
                </span>
                <span class="text-xs text-(--color-text-tertiary)">{formatTimeAgo(run.createdAt)}</span>
              </td>
              <td class="px-3 py-2">
              ...
              <td class="px-3 py-2">
                <div class="flex items-center gap-3 text-xs">
                  ...
                  {#if skippedCount(run) > 0}
                    <span class="flex items-center gap-1 text-(--color-text-tertiary)">
              ...
              <td class="px-3 py-2 text-right">
                <span class="flex items-center justify-end gap-1 text-xs text-(--color-text-tertiary)">
                  <Clock size={12} />
                  {runDuration(run)}
                </span>
              </td>
