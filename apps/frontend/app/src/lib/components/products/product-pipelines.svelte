    ...
    <div class="rounded-lg border border-border bg-surface-0">
      {#each Array(4) as _, i}
        <div class="flex items-center gap-4 px-3 py-3" class:border-t={i > 0} class:border-[var(--color-border)]={i > 0}>
          ...
          <div class="h-4 w-14 animate-pulse rounded bg-surface-2"></div>
        </div>
      {/each}
    </div>
  {:else if error}
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-border bg-surface-1">
            <th class="px-3 py-2 text-left text-xs font-medium text-(--color-text-tertiary)">ID</th>
            <th class="px-3 py-2 text-left text-xs font-medium text-(--color-text-tertiary)">Branch</th>
            <th class="px-3 py-2 text-left text-xs font-medium text-(--color-text-tertiary)">Status</th>
            <th class="px-3 py-2 text-left text-xs font-medium text-(--color-text-tertiary)">Builds</th>
        ...
        <tbody>
          {#each buildRuns as buildRun (buildRun.id)}
            <tr class="border-b border-border last:border-0 hover:bg-surface-1 transition-colors">
              ...
              <td class="px-3 py-2 font-mono text-xs text-(--color-text-secondary)">
                {buildRun.branch || '\u2014'}
              </td>
              <td class="px-3 py-2">
                <StatusBadge status={buildRun.status} />
              </td>
              <td class="px-3 py-2 text-xs text-(--color-text-secondary)">
