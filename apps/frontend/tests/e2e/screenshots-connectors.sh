#!/usr/bin/env bash
# Connectors screenshot runner (Brief B) — boots the SAME real dev-serve + vite dev as run.sh, drives
# the screenshots-connectors.mjs sweep against them (empty · add · list · dark), then tree-kills its
# servers (no orphans). One-shot; artifacts land in /tmp/ui-audit/connectors/. Never touches the
# running demo (:5173) or another session's :4380 vite — it allocates its OWN free ports.
set -Eeuo pipefail
IFS=$'\n\t'

FRONTEND_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
REPO_ROOT="$(git -C "$FRONTEND_ROOT" rev-parse --show-toplevel 2>/dev/null || echo "$FRONTEND_ROOT")"
DEV_BIN="${TMPDIR:-/tmp}/agentgateway-dev-shots-connectors"

log() { printf '\033[0;36m[shots-connectors]\033[0m %s\n' "$*"; }
err() { printf '\033[0;31m[shots-connectors:error]\033[0m %s\n' "$*" >&2; }
...
EDEN_DEV_ADDRESS="127.0.0.1:${GATEWAY_PORT}" "$DEV_BIN" >"${TMPDIR:-/tmp}/shots-connectors-devserve.log" 2>&1 &
DEV_PID=$!
...
(cd "$FRONTEND_ROOT" && EDEN_GATEWAY_TARGET="$GATEWAY_URL" \
  ...
  >"${TMPDIR:-/tmp}/shots-connectors-preview.log" 2>&1) &
PREVIEW_PID=$!
...
mkdir -p /tmp/ui-audit/connectors
log "capturing connectors gallery -> /tmp/ui-audit/connectors"
(cd "$FRONTEND_ROOT" && E2E_BASE_URL="$PREVIEW_URL" bun x node tests/e2e/screenshots-connectors.mjs)
