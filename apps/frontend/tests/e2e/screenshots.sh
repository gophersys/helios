#!/usr/bin/env bash
# W4 screenshot sweep runner — boots the SAME real dev-serve + vite dev as run.sh, drives the
# screenshots.mjs sweep against them, then tree-kills its servers (no orphans). One-shot; artifacts
# land in /tmp/ui-audit/.
set -Eeuo pipefail
IFS=$'\n\t'

FRONTEND_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
REPO_ROOT="$(git -C "$FRONTEND_ROOT" rev-parse --show-toplevel 2>/dev/null || echo "$FRONTEND_ROOT")"
DEV_BIN="${TMPDIR:-/tmp}/agentgateway-dev-shots"

log() { printf '\033[0;36m[shots]\033[0m %s\n' "$*"; }
err() { printf '\033[0;31m[shots:error]\033[0m %s\n' "$*" >&2; }
...
EDEN_DEV_ADDRESS="127.0.0.1:${GATEWAY_PORT}" "$DEV_BIN" >"${TMPDIR:-/tmp}/shots-devserve.log" 2>&1 &
DEV_PID=$!
...
(cd "$FRONTEND_ROOT" && EDEN_GATEWAY_TARGET="$GATEWAY_URL" \
  ...
  >"${TMPDIR:-/tmp}/shots-preview.log" 2>&1) &
PREVIEW_PID=$!
wait_http "$PREVIEW_URL" "vite dev"

mkdir -p /tmp/ui-audit
log "capturing screenshots -> /tmp/ui-audit"
(cd "$FRONTEND_ROOT" && E2E_BASE_URL="$PREVIEW_URL" bun x node tests/e2e/screenshots.mjs)
