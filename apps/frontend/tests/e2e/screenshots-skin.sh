#!/usr/bin/env bash
# Skin gallery runner (shadcn transposition) — boots the SAME real dev-serve + vite dev as run.sh,
# drives the screenshots-skin.mjs sweep against them (every reskinned surface, both themes), then
# tree-kills its servers. One-shot; artifacts land in /tmp/ui-audit/skin/.
set -Eeuo pipefail
IFS=$'\n\t'

FRONTEND_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
REPO_ROOT="$(git -C "$FRONTEND_ROOT" rev-parse --show-toplevel 2>/dev/null || echo "$FRONTEND_ROOT")"
DEV_BIN="${TMPDIR:-/tmp}/agentgateway-dev-skin"

log() { printf '\033[0;36m[skin]\033[0m %s\n' "$*"; }
err() { printf '\033[0;31m[skin:error]\033[0m %s\n' "$*" >&2; }
...
EDEN_DEV_ADDRESS="127.0.0.1:${GATEWAY_PORT}" "$DEV_BIN" >"${TMPDIR:-/tmp}/skin-devserve.log" 2>&1 &
DEV_PID=$!
...
(cd "$FRONTEND_ROOT" && EDEN_GATEWAY_TARGET="$GATEWAY_URL" \
  ...
  >"${TMPDIR:-/tmp}/skin-preview.log" 2>&1) &
PREVIEW_PID=$!
wait_http "$PREVIEW_URL" "vite dev"

mkdir -p /tmp/ui-audit/skin
log "capturing skin gallery -> /tmp/ui-audit/skin"
(cd "$FRONTEND_ROOT" && E2E_BASE_URL="$PREVIEW_URL" bun x node tests/e2e/screenshots-skin.mjs)
