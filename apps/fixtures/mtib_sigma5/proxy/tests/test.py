...
#     def test_cluster_creation(self, cluster_name: str, deployment_file_path: str):
#         """Test the /v1/cluster route for creating a new cluster."""
#         url = f"{self.base_url}/v1/cluster"
        
#         # Open the deployment file in binary mode
#         with open(deployment_file_path, 'rb') as deployment_file:
#             files = {'deployment_file': (deployment_file.name, deployment_file, 'application/octet-stream')}
#             data = {'name': cluster_name}
            
#             # Make the POST request with form data and the file
#             response = requests.post(url, data=data, files=files)
            
#             if response.status_code == 200:
#                 response_data = response.json()
#                 print("ClusterCreation Passed", json.dumps(response_data, indent=4))
#                 return True, response_data.get('uuid')
#             else:
#                 try:
...
#                 print(f"ClusterCreation Failed with status {response.status_code}: {formatted_error}")
#                 return False, None
...
#         self.test_cluster_creation("Test cluster name", "/workspaces/concord/apps/fixtures/mtib_sigma5/runner/deploy/deployment.yaml")
...
if __name__ == "__main__":
    tester = ClusterCreateTest()
    tester.run_all_tests("http://localhost:6969")