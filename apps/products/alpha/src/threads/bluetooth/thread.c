...
typedef enum {
    BLUETOOTH_ATTRIBUTE_PPG_INTENSITY_25HZ = 2,  // Value handle
    BLUETOOTH_ATTRIBUTE_PPG_INTENSITY_32HZ = 5,
    BLUETOOTH_ATTRIBUTE_ACCEL_25HZ = 8,
    BLUETOOTH_ATTRIBUTE_ACCEL_32HZ = 11,
    BLUETOOTH_ATTRIBUTE_PSP_PPG_INTENSITY_32HZ = 14,
    BLUETOOTH_ATTRIBUTE_PSP_ACCEL_32HZ = 17,
    BLUETOOTH_ATTRIBUTE_PSP_READ_PPG_INTENSITY_32HZ = 20,
    BLUETOOTH_ATTRIBUTE_PSP_READ_ACCEL_32HZ = 23,
...
BT_GATT_SERVICE_DEFINE(sensor_svc,
                       ...
                       BT_GATT_CHARACTERISTIC(&psp_32hz_read_accel_data_uuid.uuid,
                                              ...
                                              BT_GATT_PERM_READ,
                                              NULL, NULL, NULL),
                       BT_GATT_CCC(psp_32hz_read_accel_data_ccc_cfg_changed,
...
static bool process_event_connected(bluetooth_thread_t *p_thread) {
    ...
    k_timer_start(&_p_thread->timer_32hz, K_MSEC(32), K_NO_WAIT);  // 32Hz = 1000/32 = 32ms
    k_timer_start(&_p_thread->timer_25hz, K_MSEC(40), K_NO_WAIT);  // 25Hz = 1000/25 = 40ms
...
static bool process_event_disconnected(bluetooth_thread_t *p_thread) {
    ...
    // Reset FIFO
    while (k_fifo_get(&p_thread->data_fifo_25hz, K_NO_WAIT)) {
        ;
    }

    while (k_fifo_get(&p_thread->data_fifo_32hz, K_NO_WAIT)) {
        ;
    }
    ...
    if (err) {
        LOG_ERR("Advertising failed to start after disconnection (err %d)", err);
        return false;
    }
...
cleanup:
    ...
    if (current_second != last_second && !first_sample) {
        if (samples_this_second < 24) {
            LOG_ERR("Raw sensor 25Hz samples in last second: %d (expected: 25)", samples_this_second);
        } else {
            LOG_DBG("Raw sensor 25Hz samples in last second: %d (expected: 25)", samples_this_second);
        }
    ...
    if (interval_us > (50000 + 10000)) {  // Log any big gaps (over 50ms + 10ms)
        LOG_WRN("Event %d: gap=%.3fms (should be 40ms) process=%.3fms (25hz)",
                index,
...
cleanup:
    ...
    if (current_second != last_second && !first_sample) {
        if (samples_this_second < 31) {
            LOG_ERR("Raw sensor 32Hz samples in last second: %d (expected: 32)", samples_this_second);
        } else {
            LOG_DBG("Raw sensor 32Hz samples in last second: %d (expected: 32)", samples_this_second);
        }
        samples_this_second = 0;
        last_second = current_second;
        first_sample = false;
    ...
    if (interval_us > (40000 + 10000)) {  // Log any big gaps (over 40ms + 10ms)
        LOG_WRN("Raw sensor 32Hz event %d: gap=%.3fms (should be 31.25ms) process=%.3fms",
                index,
                (double)interval_us / 1000.0,
                (double)duration_us / 1000.0);
    }
...
cleanup:
    ...
    if (current_second != last_second && !first_sample) {
        if (samples_this_second < 31) {
            LOG_ERR("PSP Read 32Hz samples in last second: %d (expected: 32)", samples_this_second);
        } else {
            LOG_DBG("PSP Read 32Hz samples in last second: %d (expected: 32)", samples_this_second);
        }
        samples_this_second = 0;
        last_second = current_second;
        first_sample = false;
    ...
    if (interval_us > (40000 + 10000)) {  // Log any big gaps (over 40ms + 10ms)
        LOG_WRN("PSP Read 32Hz event %d: gap=%.3fms (should be 31.25ms) process=%.3fms",
                index,
                (double)interval_us / 1000.0,
                (double)duration_us / 1000.0);
    }
...
static bool process_event_timer_32hz(bluetooth_thread_t *p_thread) {
    if (!_send_raw_sensor_data_32hz(p_thread)) {
        LOG_ERR("Failed to send raw sensor data 32hz");
        return false;
    }

    if (!_send_psp_data_32hz(p_thread)) {
        LOG_ERR("Failed to send PSP data 32hz");
        return false;
    }

    if (!_send_psp_read_data_32hz(p_thread)) {
        LOG_ERR("Failed to send PSP read data 32hz");
        return false;
    }
