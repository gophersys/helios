...
from .clusters.tests.exec_uuid import (
    clusters_tests_exec_uuid_bp,
    clusters_tests_exec_uuid_socketio_handler,
)
...
class LogFilter(logging.Filter):
    ...
    def __init__(self):
        ...
        self.healthcheck_pattern = re.compile(r"GET /v1/healthcheck HTTP")
        self.deployment_pattern = re.compile(r"GET /v1/clusters/[0-9a-fA-F\-]+/deployments HTTP")
    ...
    def filter(self, record):
        ...
        if (
            self.healthcheck_pattern.search(log_message)
            or self.deployment_pattern.search(log_message)
            or self.accepted_pattern.search(log_message)
        ):
...
#                                     V1 API Routes
# -------------------------------------------------
def register_v1_routes(logger: Logger, server: Flask, socketio: SocketIO):
