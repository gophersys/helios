...
class ClusterDeploymentInfo:
    def __init__(
        self, uuid: str = "", path: str = "", status: ClusterDeploymentStatus = ClusterDeploymentStatus.NO_DEPLOYMENT
    ):
        self.uuid: str = uuid
        self.path: str = path
...
class ClusterOperatorTestEntry:
    def __init__(self, info: TestInfo, port: int, channel: grpc.Channel, stub: ClusterTestStub):
...
class ClusterOperator:
    # TODO: Add checks to ensure that control plane registry is reachable

    # Timeouts
    HOST_STARTUP_TIMEOUT_S = 180  # Linux kernel up and running
    K8S_AWAIT_NODES_TIMEOUT_S = 180  # Docker & k8s
    K8S_KUBE_DEPLOYMENT_READY_TIMEOUT_S = 180  # Default k8s deployments
    K8S_DELETE_DEPLOYMENT_TIMEOUT_S = 120  # Delete deployment
    K8S_APPLY_DEPLOYMENT_TIMEOUT_S = 500  # Apply deployment, a new image could take a while!
    ...
    def __init__(
        ...
        self.control_plane_available: bool = False
    ...
    def stop(self):
        if conf.NODES_MGMT_ENABLED:
            self._nodes_mgmt_thread_handle.join(timeout=1)
    ...
    def get_nodes_info(self) -> list:
        ...
        if not self.control_plane_available:
            ...
            for node in node_list.items:
                ...
                memory_used_bytes = memory_capacity_bytes - self._parse_node_mem_capacity(
                    node.status.allocatable["memory"]
                )
                storage_used_bytes = storage_capacity_bytes - self._parse_node_mem_capacity(
                ...
                node_info = {
                    ...
                    "cpu_cores": int(
                        node.status.capacity["cpu"].replace("m", "")
                    ),  # CPU capacity is assumed to be in millicores
            ...
            try:
                ...
                for deployment in deployments.items:
                    ...
                    deployment_info = DeploymentInfo(
                        name=deployment.metadata.name, uuid=deployment.metadata.uid, pods_info=pods_info
                    )
    ...
    def register_test(self, port: int, info: TestInfo) -> str:
        ...
        try:
            ...
            test_entry: ClusterOperatorTestEntry = ClusterOperatorTestEntry(
                info=info, port=port, channel=channel, stub=stub
            )
    ...
    def execute_test(
        self, test_uuid: str, test_config: str, test_nodes: List[str], results_cb: TestResultsCallbackType
    ) -> str:
        ...
        request = ExecuteRequest(config=test_config, nodes=test_nodes)
        ...
        try:
            ...
            if not registry_running:
                ...
                client.containers.run(
                    "registry:2", ports={f"{self.config.registry_port}/tcp": self.config.registry_port}, detach=True
                )
            else:
    ...
    def __await_for_cluster_readiness(self) -> str:
        ...
        # Readiness checks for the k8s system deployments
        error = self.__await_sys_deployments_ready()
        if error:
            return f"The cluster is an unrecoverable state due to {error}. These errros require the cluster be connected to the public internet to download updated. A system administrator must fix this."
    ...
    def __await_sys_deployments_ready(self) -> str:
        """Waits for all system deployments in the kube-system namespace to be in the 'Ready' state."""
        ...
        timeout: int = self.K8S_APPLY_DEPLOYMENT_TIMEOUT_S  # Add a timeout for waiting on deployments
        start_time = time.time()
        ...
        try:
            ...
            return f"Failed to list deployments in kube-system namespace: {str(e)}"
        ...
        for deployment in deployments.items:
            ...
            while time.time() - start_time < timeout:
                ...
                pod_list = core_v1.list_namespaced_pod(namespace="kube-system", label_selector=selector)
                all_pods_ready = True
                transitional_states = ["ContainerCreating", "Pending", "Unknown", "PodInitializing"]
                error_message = ""
                ...
                for pod in pod_list.items:
                    ...
                    if pod_status == "Ready":
                        continue
                    elif pod_status in transitional_states or pod_status.startswith("Init:"):
                        logging.info(
                            f"Pod {pod.metadata.name} of deployment {deployment_name} is in a transitional state: {pod_status}. Waiting..."
                        )
                        ...
                        return f"Pods in deployment {deployment_name} failed after initial readiness check. Error in pod {pod_in_error}, status {pod_status}: \n{logs}"

                if all_pods_ready:
                    logging.info(
                        f"Initial readiness check passed for deployment {deployment_name}. Monitoring for stability..."
                    )
                    time.sleep(post_check_delay)  # Wait to ensure pods remain stable

                    # Recheck readiness after the delay
                    are_still_ready, pod_in_error, logs = self.__are_all_pods_still_ready(
                        core_v1, deployment_name, selector
                    )
                    if are_still_ready:
                        logging.info(
                            f"All pods for deployment {deployment_name} in kube-system are confirmed stable after monitoring."
                        )
                        break  # Proceed to the next deployment
                    else:
                        logging.info(f"Rechecking pods for stability in deployment {deployment_name} after failure.")
                        return f"Pods in deployment {deployment_name} failed after initial readiness check. Error in pod {pod_in_error}: {logs}"

                time.sleep(1)  # Sleep before the next check to avoid overwhelming the API server

            if time.time() - start_time >= timeout:
                return f"Timeout {timeout}s reached while waiting for deployment {deployment_name} to be ready."
    ...
    #                                                                   Main Thread
    #  --------------------------------------------------------------------------*/
    def _nodes_mgmt_thread(self):
        # Setup
        error = self._setup_cluster()
        if error:
            logging.error(error)
        ...
        while True:
            ...
            if self.status is not ClusterStatus.RUNNING and self.proxy_connected:
                error = self.__fetch_current_cluster_deployment()
                if error:
                    logging.error(error)
                    self.error = error
                    self.status = ClusterStatus.ERRORED
        ...
        while True:
            ...
            if not self.proxy_connected:
                try:
                    ...
                    payload = {"url": self.config.grpc_server_url}  # Us

                    response = requests.post(endpoint, json=payload, timeout=100, verify=False)
                    if response.status_code == 200:
                        ...
                        logging.error(
                            f"Network error: proxy server was unable to find our URL {self.config.grpc_server_url} in the network"
                        )
                    else:
                        logging.error(
                            f"Error response from proxy, status code: {response.status_code}, response: {response.content}"
                        )
                    ...
                    if retry_counter % 10 == 0:  # Log every 10th retry attempt
                        logging.warning(
                            f"Exception occurred during cluster registration after {retry_counter} attempts: {str(e)}"
                        )
            ...
            # If we are connected to the proxy, then we keep doing health checks periodically
                ...
                try:
                    ...
                    if response.status_code != 200:
                        ...
                        logging.error(
                            f"Proxy healthcheck failed. Is the proxy down? Status code for GET {endpoint}: {response.status_code}"
                        )
        ...
        while True:
            for test in list(self.tests):
                try:
                    ...
                    logging.error(
                        f"Failed to perform health check on test at {test.port}. Unregistering from operator"
                    )
                    test.channel.close()
    ...
    #                                                            Deployment Helpers
    #  --------------------------------------------------------------------------*/
    def __fetch_current_cluster_deployment(self) -> str:
        ...
        new_deployment_uuid = json_response.get("currentDeployment")
        ...
        try:
            ...
            return f"Exception: Invalid deployment response from server: {str(e)}"
    ...
    def __wait_for_deployment_images_ready(self, deployment_name: str) -> str:
        ...
        while attempt < retry_limit:
            attempt += 1
            # Retrieve the list of pods with the specified deployment label
            ...
            for pod in pod_list.items:
                if pod.status.container_statuses:
                    for container_status in pod.status.container_statuses:
                        # Check if the container is in a waiting state with a reason in the transitional states list
                        if (
                            container_status.state.waiting
                            and container_status.state.waiting.reason in transitional_states
                        ):
                            logging.info(
                                f"Image for container {container_status.name} in pod {pod.metadata.name} is not ready. Reason: {container_status.state.waiting.reason}"
                            )
        ...
        while True:
            ...
            for pod in pod_list.items:
                ...
                if pod_status == "Ready":
                    ...
                    logging.info(f"Pod {pod.metadata.name} is in a transitional state: {pod_status}. Waiting...")
                    all_pods_ready = False
                    break  # Exit the current iteration and allow more time for transition
                else:
                    all_pods_ready = False
    ...
    def __find_error_pod_and_logs(self, core_v1, pod_list):
        time.sleep(0.5)  # allow some time for logs to be flushed
        for pod in pod_list.items:
            pod_status = self.__get_pod_status(pod)
            if pod_status != "Ready":
                try:
    ...
    def __are_all_pods_still_ready(self, core_v1, deployment_name, selector):
        ...
        for pod in pod_list.items:
            ...
            if pod_status != "Ready":
                all_pods_ready = False
                pod_in_error, logs = self.__find_error_pod_and_logs(core_v1, pod_list)
                return all_pods_ready, pod_in_error, logs
