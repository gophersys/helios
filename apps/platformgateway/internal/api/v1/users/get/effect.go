...
// the user read. It runs AFTER the 200 Envelope is written, so its error is logged, never surfaced
// (the EFFECT stage contract). The fields are redaction-safe scalars (subject + user id); a field is
// never a secret. nil provider → no Action.
	...
	return func(ctx context.Context, identity edenhttp.Identity, _ Request, output Response) error {
		provider.Emit(ctx, observability.Event{
			...
			Fields: []observability.Field{
				...
				observability.String("user-id", output.ID),
			},
		})
		return nil
	}
