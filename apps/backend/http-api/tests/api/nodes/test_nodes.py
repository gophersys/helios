...
def test_list_nodes(authed_client, mock_db):
    ...
    response = authed_client.get("/v2/nodes")
...
def test_create_node(authed_client, mock_db):
    ...
    with patch("api.v2.nodes.nodes.log_audit"):
        response = authed_client.post("/v2/nodes", data=json.dumps({
...
def test_get_node(authed_client, mock_db):
    ...
    response = authed_client.get("/v2/nodes/node-1")
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_get_node_not_found(authed_client, mock_db):
    ...
    response = authed_client.get("/v2/nodes/nonexistent")
...
def test_update_node(authed_client, mock_db):
    ...
    with patch("api.v2.nodes.nodes.log_audit"):
        response = authed_client.put("/v2/nodes/node-1", data=json.dumps({"name": "New Name"}))
...
def test_delete_node(authed_client, mock_db):
    ...
    with patch("api.v2.nodes.nodes.log_audit"):
        response = authed_client.delete("/v2/nodes/node-del")
