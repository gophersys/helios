...
def report_log_chunk(run_id: str):
    ...
    try:
        ...
        object_name = storage_key(StoragePrefixes.VALIDATION_RUNS, f"{run_id}/logs/{data.file}")
...
def get_log_file(run_id: str, file_path: str):
    ...
    try:
        storage = get_storage_client()
        bucket = get_bucket_name()
        object_name = storage_key(StoragePrefixes.VALIDATION_RUNS, f"{run_id}/logs/{file_path}")
...
def download_run(run_id: str):
    ...
    try:
        ...
        zip_object_name = storage_key(StoragePrefixes.VALIDATION_RUNS, f"{run_id}/run.zip")
...
def get_manifest(run_id: str):
    ...
    try:
        storage = get_storage_client()
        bucket = get_bucket_name()
        object_name = storage_key(StoragePrefixes.VALIDATION_RUNS, f"{run_id}/manifest.json")
