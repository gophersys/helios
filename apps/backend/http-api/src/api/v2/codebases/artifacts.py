...
from .shared import presigned_url
...
def list_artifacts(codebase_id: str, release_id: str):
    ...
    return jsonify(ApiResponse.ok([_serialize_artifact(a) for a in artifacts]).to_dict()), 200
...
def upload_artifact(codebase_id: str, release_id: str):
    ...
    storage_key = f"codebases/{codebase_id}/releases/{release_id}/{filename}"
    ...
    try:
        ...
        return internal_error(f"Failed to upload artifact: {str(e)}")
...
def download_artifact(artifact_id: str):
    ...
    if artifact.type == "UPLOAD" and artifact.storageKey:
        url = presigned_url(artifact.storageKey)
        if url:
