...
// transcript is the orchestrator role's in-process agentsession.Transcript: it assigns the
// monotonic per-session Seq (Seq == transcript offset, the agentsession invariant) and serves
// FromSeq replay. The reconcile loop opens supervisor sessions through the shared agentsession pool
// (Deps.Transcript), which appends the harness stream here. It mirrors the proven in-process
// transcript the live-local composition (internal/liveserve) and the agent-runtime composition both
// carry — the same mechanism, kept composition-local so a runnable binary imports no test package
// (agentsessiontest is test-only). Guarded by mu.
...
func (t *transcript) Append(_ context.Context, event agentsession.Event) (uint64, error) {
	...
	defer t.mu.Unlock()
	if event.SessionID == "" {
		return 0, errors.New(errors.KindInvalid, "agentgateway-orchestrator transcript: append requires a SessionID")
	}
