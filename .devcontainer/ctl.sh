...
log()  { echo -e "${GREEN}[devcontainer]${NC} $*"; }
warn() { echo -e "${YELLOW}[devcontainer]${NC} $*"; }
err()  { echo -e "${RED}[devcontainer]${NC} $*" >&2; }
info() { echo -e "${CYAN}[devcontainer]${NC} $*"; }
...
install_registry_certs() {
    ...
    if ! openssl s_client -connect "${REGISTRY_HOST}:${REGISTRY_PORT}" \
        ...
        warn "Cannot reach ${REGISTRY_HOST}:${REGISTRY_PORT} — skipping CA install"
        return 0
    fi

    info "Extracting CA chain from ${REGISTRY_HOST}:${REGISTRY_PORT}..."
    ...
    if [[ $installed -eq 0 ]]; then
        warn "No CA certificates extracted from ${REGISTRY_HOST}"
        return 0
    fi

    # Update the system trust store
    update-ca-certificates >/dev/null 2>&1
    log "Installed ${installed} CA certificate(s) from ${REGISTRY_HOST}"
}
...
install_buildkit_certs() {
    ...
    info "Injecting CA certs into buildkit container..."

    local cert_dir="/usr/local/share/ca-certificates/corekinect"
    if [[ ! -d "$cert_dir" ]] || [[ -z "$(ls -A "$cert_dir" 2>/dev/null)" ]]; then
        warn "No CA certs to inject (run install_registry_certs first)"
        return 0
    fi
    ...
    if [[ $injected -gt 0 ]]; then
        log "Injected ${injected} CA cert(s) into buildkit"
    fi
}
...
verify_docker() {
    ...
    if docker info >/dev/null 2>&1; then
        ...
        err "Docker socket not available — container builds will fail"
        return 1
    fi
}

verify_registry() {
    info "Checking container registry (${REGISTRY_HOST})..."

    if ! curl -sf --connect-timeout 5 "https://${REGISTRY_HOST}/v2/" >/dev/null 2>&1; then
        if curl -sfk --connect-timeout 5 "https://${REGISTRY_HOST}/v2/" >/dev/null 2>&1; then
            warn "Registry reachable but CA not trusted — installing certs"
            install_registry_certs
            if curl -sf --connect-timeout 5 "https://${REGISTRY_HOST}/v2/" >/dev/null 2>&1; then
                log "Registry verified after CA install"
                return 0
            fi
        fi
        warn "Registry ${REGISTRY_HOST} not reachable — image pulls/pushes will fail"
        return 1
    fi

    log "Registry ${REGISTRY_HOST} is reachable and trusted"
    return 0
}
...
verify_kubernetes() {
    ...
    if [ ! -f /root/.kube/config ]; then
        warn "No kubeconfig found at /root/.kube/config — K8s commands will fail"
        return 1
    fi
    ...
    if kubectl cluster-info >/dev/null 2>&1; then
        ...
        log "Kubernetes cluster reachable (${node_count} node(s))"
        return 0
    else
        warn "Kubeconfig exists but cluster is not reachable"
        return 1
    fi
}
...
run_preflight() {
    ...
    if [ "$failures" -eq 0 ]; then
        ...
        warn "${failures} preflight check(s) failed — some features may not work"
    fi
    echo ""
...
fix_usb_permissions() {
    ...
    if [ $fixed -gt 0 ]; then
        log "Fixed permissions for $fixed USB device(s) (J-Link/Nordic)"
    fi
}
...
create_action() {
    info "Creating action for the devcontainers"
