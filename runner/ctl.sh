#!/usr/bin/env bash
#
# ctl.sh — control script for images-runner
#
# Builds, pushes, pulls, and inspects the `+ runner` variant of a dev image:
#   ghcr.io/gophersys/<parent>-runner
#
# One Dockerfile serves every parent. Select it with RUNNER_PARENT (default: base):
#   RUNNER_PARENT=zephyr ./ctl.sh build
#
# Why the runner layer exists at all: gophersys/infrastructure docs/ci-substrate.md
#
# Multi-arch policy:
...
function cmd_build() {
  ...
  docker build --build-arg "BASE_IMAGE=${BASE_IMAGE}" -t "${IMAGE_REF}" "$PROJECT_ROOT"
}
