...
- **Alerting** — the chart's Alertmanager subchart (v0.33.0), enabled on the
  homelab since 2026-08-24, delivering to a Discord webhook via
  `webhook_url_file` from an ESO-materialized Secret. The incident-derived
  rules (NodeDiskPressure, NodeNotReady, ArcRunnersPodsFailed) live in
  `serverFiles.alerting_rules.yml` in `chart/values-homelab.yaml`. No UI is
  exposed: no Ingress, no LoadBalancer — reach it with a port-forward, the
  same posture as Prometheus itself.
...
- `platform/core/secrets-operator/` — the Alertmanager Discord webhook is an
  ExternalSecret (`chart/templates/alertmanager-discord-webhook-externalsecret.yaml`,
  vault item `shared/discord/alerts-webhook`). The OTHER credential,
  `grafana-admin`, stays imperative — see `docs/runtime-secrets.md` for the two
  reasons (2-key login, no vault item) and what would change them.
...
## TODO
- Routing by `platform.gophersys/slo-tier`
  (`.claude/rules/50-cluster-architecture.md` §8) — Alertmanager itself is on
  since 2026-08-24, with a single Discord route; the tier-aware routing tree is
  still to build.
