...
# (the F4 agent-session port: open/drive/observe/close one harness loop as an
# instrumented, resumable Eden session; the normalized Event stream + Seq-replay
# fan-out + tool-grant + credential seam; the claudeadapter spawns the real claude CLI; contract:
# docs/architecture/contracts/agentsession.md).
#
...
# `nx run agentsession:test` without reading source.
#
set -Eeuo pipefail
IFS=$'\n\t'
...
function cmd_integration() {
  require_cmd go
  log_info "integration: go test -tags integration ./... -race -count=1 (fake harness fan-out/replay + a REAL stub-binary subprocess)"
...
function cmd_cover() {
  ...
  local profile
  profile="$(mktemp -t agentsession-cover.XXXXXX)"
...
Commands:
  ...
  integration  Run the integration suite (go test -tags integration ./... -race -count=1; fan-out + real stub subprocess)
