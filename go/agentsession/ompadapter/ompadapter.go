...
// New returns the omp adapter (the ADR-0008 omp lane) configured to spawn the default "omp"
// binary. It is PURE: no env, no process, no secret.
func New() *Adapter { return NewWithConfig(Config{}) }

// NewWithConfig returns the omp adapter with an explicit Config (the integration seam for
// the stub binary). PURE.
