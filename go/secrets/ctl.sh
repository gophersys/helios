#!/usr/bin/env bash
#
# libs/go/secrets/ctl.sh — control script for the Eden secrets pattern library
# (reference->value resolution with an un-printable Secret type; depends only on
# the errors leaf library — contract: docs/architecture/contracts/secrets.md).
#
...
# `nx run secrets:test` without reading source.
#
set -Eeuo pipefail
IFS=$'\n\t'
...
function cmd_cover() {
  ...
  local profile
  profile="$(mktemp -t secrets-cover.XXXXXX)"
