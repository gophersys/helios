#!/usr/bin/env bash
#
# libs/go/observability/ctl.sh — control script for the Eden observability pattern
# library (the universal structured-Event port; OTel-aligned, secret-safe;
# stdlib-only — contract: docs/architecture/contracts/observability.md).
#
...
# `nx run observability:test` without reading source.
#
set -Eeuo pipefail
IFS=$'\n\t'
...
function cmd_lint() {
  ...
  if command -v staticcheck >/dev/null 2>&1; then
    ...
    log_warn "lint: staticcheck not installed — skipping (install: honnef.co/go/tools/cmd/staticcheck)"
  fi
  log_success "lint: OK"
}
...
function cmd_cover() {
  ...
  local profile
  profile="$(mktemp -t observability-cover.XXXXXX)"
...
Commands:
  ...
  lint    Check formatting (gofmt), run go vet, and staticcheck if installed
  vet     Run go vet ./...
