#!/usr/bin/env bash
#
# libs/go/dependencies/ctl.sh — control script for the Eden dependencies library
# (the injected-ports hexagon pattern; leaf).
#
...
function usage() {
  cat <<EOF
Usage: ./ctl.sh <command> [args...]   (dependencies — leaf=true, coverage-floor=${EDEN_COVERAGE_FLOOR}%)
