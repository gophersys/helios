#!/usr/bin/env bash
#
# libs/go/objectstorage/ctl.sh — control script for the Eden objectstorage library
# (the object-store port + a REAL MinIO/S3 adapter).
#
...
EDEN_LIB_NAME="objectstorage"
# The lib carries a REAL-substrate adapter (minioadapter): its S3 backend resolves against a REAL
# minio/minio container, proven by the //go:build integration lane — exactly the posture secrets
# adopted with vaultadapter and workspaceprovider with the docker adapter. So leaf=false: a
# substrate adapter's real behaviour is proven by integration (real MinIO), NOT by mutation
# (gremlins runs the UNIT suite GOWORK=off, which cannot reach the real minio-go transport — every
# such mutant would vacuously LIVE). The pure port types (ObjectRef/Store/taxonomy) stay heavily
# covered by the property (1000 rapid checks), conformance, canary, and unit lanes that run on
# every gate.
EDEN_LIB_LEAF="false"
# 70% floor matches the substrate-adapter posture (secrets/workspaceprovider): the adapter's real
# logic (resolve→client→Put/Get/Presign/List) is exercised through the integration lane, included
# in the cover tags below.
EDEN_COVERAGE_FLOOR="70"
EDEN_HOT_PATHS="."
# The S3 backend's real behaviour is leveraged through a REAL docker-run minio/minio container
# (docker-out-of-docker); the integration lane requires the docker CLI.
EDEN_INTEGRATION_CMDS="go docker"
# The minioadapter's resolve/client/map real logic is exercised ONLY through the REAL-substrate
# //go:build integration lane, so the per-package coverage floor MUST measure that lane too (else
# the adapter package undercounts to its fake-only unit number). The engine's sanctioned per-lib
# override.
...
function usage() {
  cat <<EOF
Usage: ./ctl.sh <command> [args...]   (objectstorage — leaf=false, coverage-floor=${EDEN_COVERAGE_FLOOR}%)
...
Commands:
  ...
  integration      REAL docker minio/minio substrate suite
