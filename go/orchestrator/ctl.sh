#!/usr/bin/env bash
#
# libs/go/orchestrator/ctl.sh — control script for the Eden orchestrator library (the S2
# basic agent orchestrator: the desired-vs-actual reconcile loop that manages agent
# spawn-up over agentsession + workspaceprovider; contract:
# docs/architecture/contracts/orchestrator.md).
#
# Usage: ./ctl.sh <command> [args...]
#
# Verbs are uniform with the monorepo's nx:run-commands convention: project.json targets
# are thin wrappers that delegate here, so a consumer can invoke
# `nx run orchestrator:test` without reading source.
#
set -Eeuo pipefail
IFS=$'\n\t'
...
function cmd_integration() {
  require_cmd go
  log_info "integration: go test -tags integration ./... -race -count=1 (concurrent reconcile, in-process fakes)"
...
function cmd_cover() {
  ...
  local profile
  profile="$(mktemp -t orchestrator-cover.XXXXXX)"
...
Commands:
  ...
  integration  Run the concurrent-reconcile integration suite (go test -tags integration ./... -race -count=1)
