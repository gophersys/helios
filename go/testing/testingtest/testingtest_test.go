...
// After honors ctx cancellation: a cancelled ctx means the channel is never sent
...
func TestFakeClock_AfterRespectsCancellation(t *testing.T) {
	...
	cancel()
	c.Advance(20 * time.Second) // crosses the deadline, but ctx is cancelled
	select {
	case v, ok := <-ch:
		if ok {
			t.Fatalf("cancelled After delivered %v; must not send", v)
		}
...
func TestNewReport_SatisfiesReport(t *testing.T) {
	var _ testingpkg.Report = testingtest.NewReport(t)
}
