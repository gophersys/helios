#!/usr/bin/env bash
#
# libs/go/testing/ctl.sh — control script for the Eden testing library
# (the conformance-suite meta-pattern; leaf (hosts AssertLifecycle)).
#
...
function usage() {
  cat <<EOF
Usage: ./ctl.sh <command> [args...]   (testing — leaf=true, coverage-floor=${EDEN_COVERAGE_FLOOR}%)
