...
for path in "$root"/* "$root"/.[!.]*; do
  name="$(basename "$path")"
  case "$name" in
    .agents|.claude|.devcontainer|.editorconfig|.gitattributes|.gitignore|.omp|.yarnrc.yml|AGENTS.md|CLAUDE.md|README.md|docs|harnesses|nx.json|package.json|project.json|scripts|secrets|yarn.lock) ;;
