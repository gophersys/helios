...
This document makes the Stage 1 concepts from [00-validation-philosophy.md](../../vision/validation-philosophy.md) Section 3.1 concrete by walking through real test implementations for Alpha. It covers:
...
| 3 -- Integration tests | Instrumented firmware on real product board | MTIB product fixture | See [10-stage3-alpha-example.md](./stage3.md) |
...
No `needsMtib`, no `targets`, no `fixture_type` -- those fields only appear in Stage 2+ configurations. The full `pipeline.yaml` schema is defined in [arch-stage1-software-tests.md](../../architecture/validation/stage1-software-tests.md) Section 5.4.
