...
### Required Files (MinIO Paths)

Firmware artifacts are stored in MinIO under the `concord` bucket:

**v0.5.0 MFG Base Firmware** (flash via J-Link):
```
firmware/builds/alpha_mfg_fw/cmmk9o90200028785zcfxqlxe/
  0.5.0_no_debug_app_nrf52840.hex
  0.5.0_no_debug_comms_nrf9151.hex
  0.5.0_no_debug_108.0.5.0.cfw
  0.5.0_no_debug_109.0.5.0.cfw
```

**v0.5.1 MFG Target Firmware** (FUOTA target):
```
firmware/builds/alpha_mfg_fw/cmmk9o90v00048785vl0v4od5/
  0.5.1_no_debug_app_nrf52840.hex
  0.5.1_no_debug_comms_nrf9151.hex
  0.5.1_no_debug_108.0.5.1.cfw
  0.5.1_no_debug_109.0.5.1.cfw
```

**CRITICAL**: Use matching track builds. The v0.5.0 and v0.5.1 builds above are both
`no_debug` (no track suffix in CFW filename). Do NOT mix with `-BM` or `-BMD` builds.
...
### profileId shows null in Search (NOT a bug)

**Status (2026-03-11): RESOLVED - Display issue only**

The `GET /api/System/Devices/Search` endpoint returns `profileId: null` for ALL devices.
This is a display/schema issue in the API response, NOT a missing profile.

**Profiles ARE created** - verify using the dedicated profiles endpoint:
```python
resp = sess.get(f"{base_url}/System/Devices/Sessions/Profiles",
                headers=headers, json={"deviceIds": [device_id]})
profiles = resp.json()  # List of {deviceId, publicKey}
```

DevicePersonalizer now verifies using this endpoint instead of checking profileId.
...
Run the complete FUOTA test:

```bash
# Full automated test (downloads from MinIO, flashes, personalizes, FUOTA)
cd /workspaces/concord
export PYTHONPATH=libs/python:libs/protocols:libs
pytest apps/validation/alpha/tests/fuota/test_fuota.py -v -s \
    --mtib-addr 10.4.45.33 \
    --device-snr 09J5
```

The test will:
1. Download v0.5.0 hex files from MinIO
2. Flash device via J-Link
3. Power cycle and verify boot
4. Personalize device (with profile verification)
5. Upload v0.5.1 CFW to CoreCloud
6. Create FUOTA plan and assign device
7. Wait for FUOTA completion (up to 90 min)
8. Verify firmware version after update

## Manual Scripts

**Setup script** (flash + personalize + create plan):
```bash
python apps/validation/alpha/scripts/fuota_setup.py \
    --mtib 10.4.45.33 --snr 09J5
```

**Monitor script** (UART + API polling):
```bash
python apps/validation/alpha/scripts/monitor_fuota.py \
    --mtib 10.4.45.33 --device 70B3D584C01E1DDD --poll 30
```
