...
| **DELETED, not archived** — `architecture/13-versioning-and-git-workflow.md` | —                    | `.claude/rules/git-process.md` (ADR-0032). Deleted rather than moved to the attic, on Mateo's no-legacy ruling D6 of 2026-08-25 quoted in that ADR — the one exception to `CLAUDE.md`'s "never delete it". Its §7 per-class-versioning synthesis has **no successor by decision**; the four homes it pointed at survive (11 §4, 10 §8, 06 §3, 09 §4/ADR-0016). Git history holds the content. |
