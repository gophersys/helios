# Phase 1 — Backend: Generic Resource YAML Get/Apply Service

## Objective

Create a generic service that can fetch any supported K8s resource as YAML and apply edited YAML back. This powers the "View YAML" and "Edit YAML" functionality across all resource detail pages.

---

## 1. Create `src/services/kubernetes/resources.py`
...
from .serializers import _isoformat


# Map of supported resource kinds to their API methods
# (read_func_factory, apply_func_factory, delete_func_factory)
# Each factory takes the API client and returns (read_fn, apply_fn, delete_fn)
...
def apply_resource_yaml(kind: str, namespace: str, name: str, yaml_str: str) -> dict:
    ...
    try:
        result = patch_fn(name, namespace, body=body)
    except ApiException as e:
...
1. `python3 -m py_compile src/services/kubernetes/resources.py` — passes
2. `pyyaml` is available (bundled with kubernetes client dependency)
3. The `get_k8s_client()` function exists from Tier 1 Phase 1
