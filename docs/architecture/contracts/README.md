...
| [agentprofile.md](agentprofile.md) **DRAFT** | kernel library (S4 seam; the gate arm is S5) | The single source of truth for agent INSTRUMENTATION: the role × harness schema (claude/omp/codex per ADR-0021), the composition and precedence of rules and skills, per-repository overlays, and a DETERMINISTIC renderer that emits each harness's native files — committed, addressable by digest, and drift-gated at commit time (Mateo's rulings 1 and 2, 2026-08-26). It disambiguates `orchestrator.md`'s `agentconfiguration/TemplateStore` `SkillRef`/`RuleRef` resolution; `agentconfiguration` keeps `RouteKey → Route`. NOT frozen — the boundary and the render channel are the open questions |
| [fleettelemetry.md](fleettelemetry.md) **DRAFT** | service library (S6 seam, plane b) | The fleet message → OTel mapping: every message kind to a span, span event or link with its attributes; task-as-trace with per-session root spans joined by propagated context; the DURABLE bus consumer that never competes with a delivery consumer; and the UI streaming read model served from that same consumer (Mateo's ruling 3 in full, 2026-08-26; ruling 4's TRACE half, while its task-id MECHANISM is referred back to him — `fleetenvelope` carries no `TaskID` field and this contract declines to mint a second one). CITES `observability` for the port, and `fleetenvelope` + `fleetbus` for the header and the consumer mechanics; owns none of them. ⚠️ **The latter two are NOT on `main` yet** — DRAFTs on the sibling branch `docs/fleet-contracts` (`70a979f`) — so those citations resolve only once that branch merges; the stated order is `fleetenvelope` → `fleetbus` → `fleettelemetry`. NOT frozen — and no `oteladapter` exists and no eden workload has an OTLP endpoint |
