...
> Status: **Frozen — revised (ADR-0016 base + ADR-0022 §4, Milestone-B B2)** · 2026-06-14 · The
> frozen surface was EXTENDED (additive-only) with the supervising-provider + Entrypoint/workload-pod
> capability (decision #4, resolving OD-15 option-a): un-froze → extended §7 (Q15/Q16) → re-recorded
> `.apibaseline` → re-froze (ADR-0016 §1). The additions reconcile with — never contradict — the
> ratified Q13/Q14 amendments below. See §7 Q15 (supervision + reconcile-from-reality) and Q16
> (Entrypoint/workload-pod + the k8s OOM-discriminator, closing OD-15). The full ADR-0020 8-dimension
> gate is GREEN on real docker + k3d with the additions. Original frozen baseline below.
>
...
It owns exactly **five** things (the fifth added in the Milestone-B B2 revision, ADR-0022 §4):
...
4. **Configurability** — which substrate adapter serves a request, and the per-workspace spec
   (image/template, mounts, limits, egress, tenancy keys, **and the optional `Entrypoint`** that
   makes the container's main process the workload — PID-1 — per the workload-pod capability),
   resolved upstream and handed in frozen.
5. **Runtime supervision** (the ADR-0022 §4 "fat" provider, §7 Q15/Q16) — supervise the supervised
   workspaces via a global label-filtered watch (docker events / k8s pod-watch), **normalize** docker
   actions and k8s pod phases into ONE platform-neutral `Event`/`Status` space (reusing the existing
   `State`/`Condition` vocabulary), and **reconcile-from-reality** (list-by-label re-adoption on
   restart, so the supervised set is rebuilt from the live substrate, never in-memory-only truth). The
   orchestrator **Probes** the supervised `Status` (the docker/k8s API = the *hard* lifecycle) rather
   than raw heartbeats. This is the `Supervisor` port (`Supervise`/`Supervised`/`Reconcile`), split
   from `Provider` along the provision-vs-supervise seam.
...
| Q15 | **✅ CONTRACT REVISION (ADR-0022 §4, Milestone-B B2) — the SUPERVISING ("fat") provider.** The provider ABSORBS runtime supervision (IOTEA's `manager.Manager` shape): a global label-filtered watch (docker events on the docker adapter, a k8s pod-watch on the kubernetes adapter), NORMALIZED into ONE platform-neutral `Event`/`Status` space, plus reconcile-from-reality (list-by-label re-adoption on restart). What is the smallest additive exported surface? | (n/a — ratified intent from ADR-0022 §4, not an original-draft fork.) | (n/a.) | 🧩 **Resolved additively (no break to the frozen surface).** New consumer-facing port **`Supervisor`** (3 methods, ≤5 ceiling, split from `Provider` along the provision-vs-supervise seam) implemented by the concrete `*Provisioner` IN ADDITION to `Provider`: `Supervise(ctx, Selector) (<-chan Event, error)` (the normalized stream; reconciles-from-reality first, then streams; closes on ctx cancel; `UnsupportedError` if no routable adapter declares `CapSupervise`), `Supervised(ctx, Handle) (Status, error)` (the queryable supervised Status the orchestrator Probes — read from the LIVE substrate via the adapter Probe, never an in-memory cache; `NotFoundError` when gone), `Reconcile(ctx, Selector) ([]Descriptor, error)` (the list-by-label re-adoption primitive). New value type **`Event`** (`Handle`/`Kind`/`State`/`Condition`/`Detail`/`At`) reusing the EXISTING `State`/`Condition` vocabulary (never a parallel one). New **`EventKind`** closed enum (`EventStarted`/`EventStopping`/`EventStopped`/`EventFailed`/`EventRemoved`) — the IOTEA `runtime.Event` shape adapted; the library owns the ONE Action→EventKind normalization table. New OPTIONAL adapter seam **`Watcher`** (1 method `Watch(ctx, Selector) (<-chan WatchEvent, error)`) the adapter implements when it declares `CapSupervise`; the supervising provider type-asserts for it. New value type **`WatchEvent`** (the adapter's RAW reading the library normalizes). New capability **`CapSupervise`**. All additive — apidiff confirms additive-only (no removal/change to `Provider`/`Workspace`/`Run`/`Files`/`Adapter`/the error set/the frozen value types). Proven on real docker (the daemon event stream) and real k3d (the cross-namespace pod-watch): reconcile-from-reality re-adopts a live workspace after a fresh-Provider restart; the watch goroutine is leak-free (goleak). |
| Q16 | **✅ CONTRACT REVISION (ADR-0022 §4, Milestone-B B2; resolves OD-15 option-a) — the ENTRYPOINT/workload-pod capability.** The container's MAIN process IS the workload (PID-1) on docker AND kubernetes — native liveness/restart/OOM; on kubernetes the kubelet's `OOMKilled` container-status reason attaches to the READABLE workspace container, surfacing the `ConditionOOMKilled` discriminator NATIVELY (closing the Q14/OD-15 exec-into-hold gap). How does this reconcile with Q14, and what is the additive surface? | (n/a — ratified intent, OD-15 option-a.) | (n/a.) | 🧩 **Resolved additively; reconciled with Q14.** ONE new `WorkspaceSpec` field **`Entrypoint []string`**: when non-empty the adapter sets it as the container `Cmd` / pod `Command` (PID-1 = the workload) instead of the long-lived `sleep infinity` hold; when empty (the default) the EXEC-INTO-HOLD model of Q14 is verbatim — so every existing consumer is unaffected and Q14's honest exec-into-hold limitation still stands for specs that do NOT set `Entrypoint`. New capability **`CapWorkloadPod`** (both v1 adapters declare `CapFull`). The OD-15 gap Q14 documented is now CLOSED **for Entrypoint specs**: because the workload is PID-1 of the readable container, docker's `State.OOMKilled` and the kubelet's container-status `OOMKilled` reason both attach to the container the adapter reads — so `ConditionOOMKilled` surfaces on the supervised Status on BOTH substrates (Q14's residual kubernetes gap was specifically the exec-into-hold child; the workload-pod model is the "workload-owned object" future-work Q14 itself named). Proven on real docker (`TestDocker_EntrypointWorkloadIsPID1OOM` — the PID-1 memory-bomb's cgroup kill surfaces `ConditionOOMKilled` via `Supervised`, no skip) and exercised on real k3d (`TestK3d_EntrypointWorkloadIsPID1OOM`, honest-skip where the node does not enforce the cgroup). Additive — apidiff confirms no break (the `Entrypoint` field, `CapWorkloadPod`, and `CapSupervise` are additions to existing types). |
