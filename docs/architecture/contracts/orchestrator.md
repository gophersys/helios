...
| Q11 | **⚠️ POST-FREEZE DRAFT AMENDMENT — the `Agent` record gained two fields the frozen §4 did not carry (raised in implementation/adversarial verification; Mateo ratifies).** The frozen §4 `Agent` struct encoded desired-vs-actual using `Status` alone (with `StatusStopping` standing in for "a Stop was recorded"), and put `Cluster` on `SpawnRequest`/`ProvisionError` but NOT on the record. The landed `*Pool` adds **`Agent.Desired Desired`** (a new closed enum `Desired{DesiredRunning, DesiredStopped}`) and **`Agent.Cluster ClusterRef`** to the record. | (n/a — surfaced during the TDD/adversarial implementation, not in either original draft.) | (n/a.) | 🧩 **Amended in draft (pending Mateo's ratification).** Both additions are load-bearing for the reconcile spine and are purely ADDITIVE to the serializable record (still a plain value holding only loggable scalars + the `workspaceprovider.Handle` + the opaque `SessionRef` — rationale 2 is untouched; no secret, no live handle). **(1) `Desired`** makes the desired-vs-actual split a FIELD rather than overloading `Status`: `Stop` records `DesiredStopped` and a *separate* `Status` tracks the observed actual, so the loop's "re-read the desired record under the lock so a concurrent Stop/Resume wins" (the race-clean property) reads one unambiguous intent field instead of inferring intent from a status value that also means "actual is stopping." Without it, a Running→Suspended drop could not be distinguished from a Stop at the record level. The frozen 8-value `Status` (Q6) is unchanged; `Desired` is orthogonal. **(2) `Agent.Cluster`** persists the cluster the workspace was provisioned on (resolved at `Spawn` from `SpawnRequest.Cluster` ∨ `Config.DefaultCluster`), so reconcile re-derives the same `WorkspaceSpec` on a retried/post-restart pass and `ProvisionError.Cluster` carries the right value — the workspace half of rationale 2's restart-correctness needs the cluster on the durable record, not only on the transient request. Recorded here so the field additions are a visible, ratifiable amendment rather than a silent divergence between the frozen §4 and the landed code (ADR-0016 §1: an edit to a frozen surface is a negotiation, never silent). A consumer that reads only the frozen §4 fields is unaffected (both new fields are appended, zero-valued by default). |
