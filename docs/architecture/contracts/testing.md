...
| 4 | `Clock` shape: producer `Now/Since/After/Sleep(ctx)` vs consumer `Now/Since/NewTimer`. | `After(d) <-chan` + `Sleep(ctx, d) error`. | `NewTimer(d) (<-chan, func() bool)` (stoppable), no `Sleep`. | 🧩 **Merged to 4 methods:** `Now`, `Since`, `NewTimer` (consumer — the stop func prevents fake-timer leaks; `After` is `NewTimer` minus the stop), `Sleep(ctx, d) error` (producer — the cancellable blocking seam, honoring ctx-first 10 §9). Under the 5-method ceiling; no split needed. |
