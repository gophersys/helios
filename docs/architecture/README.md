...
| Fleet telemetry (the message-kind → OTel span/event/link mapping and its attribute vocabulary, task-as-trace, the durable bus consumer, the UI streaming read model) | `contracts/fleettelemetry.md` **DRAFT** — cites `contracts/fleetenvelope.md` for the header and `contracts/observability.md` for the port; the S6 plane (b) of 03 §1. It does NOT own the wire (that is `fleetenvelope`/`fleetbus`) nor the `Provider` port |
