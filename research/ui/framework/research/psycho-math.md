...
are checked from both sides (`tools/ui/tests/test_axes.py`): a named symbol
...
A7  all box edges are integers.  → ui.audit.check_integer_edges
A8  every adjacent hit-target pair: 24 px circles on their bounding boxes do not
      intersect (=> centre-to-centre pitch >= 24 px).  → ui.audit.check_hit_pitch
...
A12 minimise Omega = -N*sum(p_i*log2 p_i) over edge/size classes; report the count
      ...
      → ui.audit.axis_census counts and reports it;
      → ui.audit.check_axis_budget spends it (controls per CONTROL x-axis >= 3).
