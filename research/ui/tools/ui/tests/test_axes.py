...
Imports of the new symbols are attribute lookups on `ui.audit` inside each
test on purpose: a module-level `from ui.audit import axis_census` would
...
# ui symbol after an arrow — `A7  all box edges are integers.
# → ui.audit.check_integer_edges`. One token, greppable from either side,
# and resolvable, which is what makes the claim checkable rather than decorative.
IMPLEMENTED_BY = re.compile(r"→\s*(ui\.[\w.]+)")
...
W2_MARKS = {
    "A7": ({"ui.audit.check_integer_edges"}, "integer"),
    "A8": ({"ui.audit.check_hit_pitch"}, "24 px"),
    "A12": ({"ui.audit.axis_census", "ui.audit.check_axis_budget"}, "Omega"),
}
...
def test_axis_budget_override_without_a_reason_is_a_spec_error(font_path):
    ...
    validates a panel spec, and rules_from, which is what `ui audit` and
    `ui score` actually call — a rule enforced on only one of the two is
    enforced on neither."""
    with pytest.raises(SpecError) as exc:
...
def test_corpus_seeds_exist_for_the_three_new_classes():
    ...
    `ui score` reports seed_caught per class, and that field means nothing
...
def test_marked_research_rows_name_a_live_consumer():
    ...
    names `ui.audit.check_axes` is a believed check in prose form, and it
