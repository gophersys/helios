...
Imports of `check_ratios` are attribute lookups on `ui.audit` inside each
test on purpose: a module-level `from ui.audit import check_ratios` would
...
def test_audit_cli_fails_on_a_wrong_declared_ratio_row(audit_fixture, capsys):
    ...
    says 45, and `ui audit` said nothing at all."""
...
def test_size_ratio_class_is_measured_by_check_ratios():
    ...
    assert row.predicate == "ui.audit.check_ratios", row.predicate
