"""ui.audit — the proof battery over probe output (LAYOUT-MATH).

Every check consumes {"containers": [...], "parts": [...]} as produced by
ui.probe.collect and returns a list of failure strings naming both parts
...
def check_axis_budget(parts: list[Part], rules: Rules) -> list[str]:
    ...
    `axis_budget_exempt` with its reason instead, which ui.score reports by
