...
The session creation wizard asks which test package to run. Pick a released package for production batches, or a development package for dry-running an unreleased build on a dev fixture. The fixture's `purpose` (`DEV` or `RELEASE`) gates the choice asymmetrically: production fixtures (`purpose=RELEASE`) only accept released packages; dev rigs (`purpose=DEV`) accept both, so the same physical rig can run dev iterations and verify the released build before it ships to the floor. Without an explicit pick the wizard auto-fills the latest released package; if none exists for this product the request is rejected with a 409 — release a package first or pass `testPackageId` explicitly.
