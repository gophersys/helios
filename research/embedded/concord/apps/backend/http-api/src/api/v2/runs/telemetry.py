"""Telemetry API endpoints for post-analysis of test runs.

Serves telemetry manifest and per-channel JSONL data written by
TelemetryStreamer during test execution.

Endpoints:
  GET /v2/runs/<id>/telemetry/manifest  — Telemetry manifest (JSON)
  GET /v2/runs/<id>/telemetry/<channel> — Channel data (JSONL)
"""
import json
import logging
...
def get_telemetry_manifest(run_id: str):
    """GET /v2/runs/<id>/telemetry/manifest
...
def get_telemetry_channel(run_id: str, channel: str):
    """GET /v2/runs/<id>/telemetry/<channel>
