...
-- 1b. Drop the legacy fixture_designs.testPackageId column added by
-- 20260409220000_fixture_design_extensions. It was declared with
-- onDelete: SetNull and never referenced in the Prisma schema after
-- a later schema cleanup, so the column persisted in the DB but was
-- effectively dead. We re-create it below with the
-- per-package-ownership semantics this refactor needs (Cascade,
-- @unique, NOT NULL after backfill). Idempotent so retries on a
-- partially-applied migration succeed.
ALTER TABLE "fixture_designs" DROP CONSTRAINT IF EXISTS "fixture_designs_testPackageId_fkey";
DROP INDEX IF EXISTS "fixture_designs_testPackageId_idx";
ALTER TABLE "fixture_designs" DROP COLUMN IF EXISTS "testPackageId";

-- 2. New columns (all nullable / defaulted so adding them is safe).
-- IF NOT EXISTS guards let the migration retry cleanly if it failed
-- after committing a subset on a prior attempt.
ALTER TABLE "fixtures" ADD COLUMN IF NOT EXISTS "purpose" "FixturePurpose" NOT NULL DEFAULT 'RELEASE';
ALTER TABLE "manufacturing_sessions" ADD COLUMN IF NOT EXISTS "testPackageId" TEXT;
ALTER TABLE "product_stage_configs" ADD COLUMN IF NOT EXISTS "releasedTestPackageId" TEXT;
ALTER TABLE "fixture_designs" ADD COLUMN "testPackageId" TEXT;
ALTER TABLE "fixture_designs" ADD COLUMN IF NOT EXISTS "status" "TestPackageStatus";
...
ALTER TABLE "fixture_designs" DROP CONSTRAINT IF EXISTS "fixture_designs_name_revision_key";
CREATE UNIQUE INDEX IF NOT EXISTS "fixture_designs_testPackageId_key" ON "fixture_designs"("testPackageId");
CREATE INDEX IF NOT EXISTS "fixture_designs_status_idx" ON "fixture_designs"("status");
...
ALTER TABLE "test_packages" DROP COLUMN IF EXISTS "fixtureDesignId";

-- 7. Drop dead v1 fields.
ALTER TABLE "test_packages" DROP COLUMN IF EXISTS "stagesEnabled";
ALTER TABLE "test_packages" DROP COLUMN IF EXISTS "gitDirty";
...
EXCEPTION
    WHEN undefined_column THEN NULL;  -- already renamed by a prior partial run
END $$;
...
EXCEPTION
    WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS "manufacturing_sessions_testPackageId_idx" ON "manufacturing_sessions"("testPackageId");
...
EXCEPTION
    WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS "product_stage_configs_releasedTestPackageId_idx" ON "product_stage_configs"("releasedTestPackageId");
