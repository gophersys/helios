...
class Schematic():
    ...
    """The ``embedded_fonts`` token defines whether fonts are embedded in the file.
    Available since KiCad v9."""
    ...
    def from_sexpr(cls, exp: list) -> Schematic:
        ...
        for item in exp:
            ...
            # KiCad 9+ adds generator_version token
            ...
            # KiCad 9+ tokens — store if present, ignore gracefully
            if item[0] == 'embedded_fonts': object.embeddedFonts = (item[1] == 'yes') if len(item) > 1 else True
        return object
