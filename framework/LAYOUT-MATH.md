...
| Ratio table | `densui.audit.check_ratios`: declared rows vs the rendered boxes (median per kind, instance spread its own failure; `span` rows claim a distance) | drift toward library defaults |
