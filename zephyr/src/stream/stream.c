...
int32_t cipher_stream_send_pattern(cipher_daemon_t *d, uint16_t device_id, uint16_t stream_id,
    ...
    // Generate the payload one chunk at a time (deterministic pattern) and fold
    // the checksum as we go — so an arbitrarily large stream (100 MB+) can be
    // sent from a node holding only a single chunk in RAM. The receiver checksums
    // the bytes it reassembles, which are exactly these generated bytes, so the
    // END checksum matches regardless of the pattern.
    uint8_t chunk[CONFIG_MAX_PAYLOAD_SIZE - sizeof(cipher_header_t) - 4];
    ...
    while (sent < length) {
        ...
        for (uint16_t i = 0; i < n; i++) {
            chunk[i] = (uint8_t)((sent + i) * 31 + 7);
        }
        checksum = cipher_stream_fnv1a(checksum, chunk, n);
        if (!send_stream_packet(d, iface, device_id, stream_id, CIPHER_FLAG_STREAM_DATA, seq,
                                chunk, n)) {
            return (int32_t)sent;
        }
