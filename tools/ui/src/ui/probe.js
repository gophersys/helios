/* ui probe — measures a rendered page's true geometry.
 * Called by ui.probe.collect(), which appends `uiProbe(cfg)` after
 ...
 * glyphs do not (LAYOUT-MATH A-4). Output: a pre element (id ui-probe).
 */
function uiProbe(cfg) {
  'use strict';
  /* The string whose advance identifies a face, byte-identical to
   * ui.audit.FONT_SENTINEL (a test pins that): the expectation is computed
   ...
   * ui.audit.FONT_IDENTITY_SIZE_PX. Which face was drawn is not a fact about
  ...
  function collect() {
    if (document.getElementById('ui-probe')) return;
    var root = document.querySelector(cfg.root);
    if (!root) throw new Error('ui-probe: missing root ' + cfg.root);
