...
def cmd_audit(args) -> int:
    ...
    try:
        ...
        out = probe.collect(
            args.page, root=pr["root"],
            containers=pr.get("containers", {}), parts=pr.get("parts", {}),
            text_kinds=set(pr.get("text_kinds", [])),
            owner_attr=pr.get("owner_attr", "data-addr"),
            root_width=pr.get("root_width"), extra_js=sweep_js)
...
def cmd_zoom(args) -> int:
    ...
    measure.zoom(img, box, args.out, scale=args.scale,
                 grid_step=args.grid, grid_divisor=args.grid_divisor)
...
def cmd_compare(args) -> int:
    ...
    for spec in args.region:
        ...
        regions[name] = box
    written = compare.side_by_side(measure.load(args.ref), measure.load(args.built),
                                   regions, args.out_dir, scale=args.scale)
...
def main(argv: list[str] | None = None) -> int:
    ...
    c.add_argument("--region", action="append", required=True,
