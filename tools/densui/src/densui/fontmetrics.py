...
def resolve_path(declared: str | None = None) -> str:
    ...
    densui.audit.check_font_identity would fail for a reason that is not a
    defect."""
    for cand in (os.environ.get("DENSUI_FONT"), declared, *PRESENT_FALLBACKS):
        if cand and pathlib.Path(cand).exists():
            return cand
    raise FontError(
        f"no usable font: DENSUI_FONT unset or absent, declared {declared!r} absent, "
        f"and none of {PRESENT_FALLBACKS} exists — set DENSUI_FONT"
    )
