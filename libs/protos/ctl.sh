...
# 'generate' operation generates Python and protobuf code from .proto files located in libs/protos,
...
# 'clean' operation removes all generated Python and C files except for __init__.py within libs/protos.
        ...
        for protofile in "${proto_files[@]}"; do
            ...
            for genfile in $generated_files; do
                ...
                if [[ ! -z "$package_name" ]]; then
                    sed -i'' -e "s/import \([^ ]*\)_pb2 as \([^ ]*\)/import protos.\1.\1_pb2 as \2/g" "$genfile" || echo "Failed to modify $genfile"
                    sed -i'' -e "s/from \([^ ]*\) import \([^ ]*\)_pb2 as \([^ ]*\)/from protos.\1 import \2_pb2 as \3/g" "$genfile" || echo "Failed to modify $genfile"
                fi
