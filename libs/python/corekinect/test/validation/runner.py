...
class ProductContext:
    ...
    Loaded from /v2/catalog/products/by-slug/<slug> at session start.
    ...
    def from_api(cls, slug: str, api_url: str, api_key: str) -> "ProductContext":
        ...
        resp = requests.get(
            f"{api_url}/v2/catalog/products/by-slug/{slug}",
            headers=headers,
            timeout=30,
        )
...
class RunReporter:
    ...
    def _post(self, endpoint: str, data: Dict[str, Any]) -> Optional[Dict]:
        ...
        url = f"{self.api_url}/v2/validation/runs/{self.run_id}{endpoint}"
        try:
