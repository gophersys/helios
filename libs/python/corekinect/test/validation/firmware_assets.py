...
Downloads firmware from MinIO storage and uploads to MTIB server for flashing.
Tracks uploaded files for automatic cleanup after tests complete.

Integration with existing patterns:
- Uses same MinIO config as ArtifactWriter (STORAGE_URL, etc.)
- Works with FixtureController for upload/flash operations
- Can be used standalone or as TestContext component
...
Example usage:
    ...
    # ... flash using hex_name ...
    assets.cleanup()

    # With TestContext
    ctx.firmware.fetch_and_upload(storage_key)
    ctx.fixture.flash_firmware(hex_name)
"""
