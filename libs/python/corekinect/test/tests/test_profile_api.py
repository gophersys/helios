    ...
    def test_from_env_prefers_api_over_file(self, base_env, sample_profile_data):
        ...
        with patch.dict(os.environ, env, clear=True):
            with patch("requests.get", return_value=mock_response) as mock_get:
                ...
                with patch("corekinect.test.context.MtibV1Client"):
                    with patch("corekinect.test.context.CloudClient"):
                        with patch("corekinect.test.context.FixtureController"):
                            with patch("corekinect.test.context.UartDemuxer"):
