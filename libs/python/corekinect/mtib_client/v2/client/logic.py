...
class LogicMixin(BaseClient):
    ...
    def logic_capture_start(
        self, config: LogicCaptureConfig
    ) -> Tuple[Optional[str], Optional[str]]:
        """Start a logic capture.

        Args:
            config: LogicCaptureConfig protobuf message with channel and trigger settings.
        ...
        try:
            resp = self._call(
                "LogicCaptureStart", LogicCaptureStartRequest(config=config)
            )
            if not resp.success:
    ...
    def logic_capture_status(
        ...
        try:
            resp = self._call(
                "LogicCaptureStatus", LogicCaptureStatusRequest(capture_id=capture_id)
            )
            if not resp.success:
    ...
    def logic_capture_stop(self, capture_id: str) -> Optional[str]:
        ...
        try:
            resp = self._call(
                "LogicCaptureStop", LogicCaptureStopRequest(capture_id=capture_id)
            )
            if not resp.success:
                return resp.message
            return None
