 ...
 *                                                                                               Public
 *---------------------------------------------------------------------------------------------------*/
void cipher_rpc_handler(cipher_daemon_t *d, cipher_rpc_entry_t *entry) {
    ...
    k_fifo_put(&d->rpc.local_request_event_queue, fifo_item);
...
void cipher_rpc_thread_add_event(cipher_daemon_t *d, rpc_event_t *event) {
    ...
    rpc_event_t *local_event = k_heap_aligned_alloc(&d->rpc.heap, 8, sizeof(rpc_event_t), K_NO_WAIT);
    CHECK_MALLOC(local_event);
    local_event->options = k_heap_aligned_alloc(&d->rpc.heap, 8, option_size, K_NO_WAIT);
    ...
    k_fifo_put(&d->rpc.ctrl_event_queue, local_event);
}
