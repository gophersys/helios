#!/usr/bin/env bash
#
# libs/go/errors/ctl.sh — control script for the Eden errors pattern library
# (the universal typed-error model; LEAF library, stdlib-only — contract:
# docs/architecture/contracts/errors.md).
#
...
# `nx run errors:phase-gate` without reading source.
#
set -Eeuo pipefail
IFS=$'\n\t'
...
EDEN_LIB_NAME="errors"            # the slug
EDEN_LIB_LEAF="true"             # pure leaf, stdlib-only → mutation lane applies
EDEN_COVERAGE_FLOOR="80"         # leaf-lib floor (ADR-0018); FLOOR, not a target
EDEN_HOT_PATHS="."               # hot benchmark name regexp (New spine, Kind round-trip)
EDEN_INTEGRATION_CMDS="go"        # a stdlib-only leaf has no real substrate; go suffices
...
function usage() {
  cat <<EOF
Usage: ./ctl.sh <command> [args...]   (errors — leaf, coverage-floor=${EDEN_COVERAGE_FLOOR}%)
...
Commands:
  ...
  integration      REAL substrate suite (a leaf lib has none → go suffices)
