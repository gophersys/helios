#!/usr/bin/env bash
#
# libs/go/observability/ctl.sh — control script for the Eden observability library
# (the telemetry sink pattern; leaf).
#
...
function usage() {
  cat <<EOF
Usage: ./ctl.sh <command> [args...]   (observability — leaf=true, coverage-floor=${EDEN_COVERAGE_FLOOR}%)
