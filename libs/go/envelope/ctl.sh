#!/usr/bin/env bash
#
# libs/go/envelope/ctl.sh — control script for the Eden envelope-encryption library
# (the AES-256-GCM per-secret-DEK / external-KEK primitive; LEAF library — contract:
# docs/architecture/contracts/envelope.md, ADR-0029).
#
...
# `nx run envelope:phase-gate` without reading source.
#
set -Eeuo pipefail
IFS=$'\n\t'
...
EDEN_LIB_NAME="envelope"          # the slug
EDEN_LIB_LEAF="true"             # pure leaf (crypto/* + stdlib + errors + secrets) → mutation lane applies
EDEN_COVERAGE_FLOOR="80"         # leaf-lib floor (ADR-0018); FLOOR, not a target
EDEN_HOT_PATHS="."               # hot benchmark name regexp (Seal/Unseal spine, Fingerprint)
EDEN_INTEGRATION_CMDS="go"        # a crypto leaf has no real substrate (the KEK arrives via a fake/real Provider); go suffices
...
function usage() {
  cat <<EOF
Usage: ./ctl.sh <command> [args...]   (envelope — leaf, coverage-floor=${EDEN_COVERAGE_FLOOR}%)
...
Commands:
  ...
  integration      REAL substrate suite (a crypto leaf has none → go suffices)
