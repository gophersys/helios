// Package deploy is the http-gateway template's deploy surface: the ONE typed ServiceSpec the
...
// Spec returns the http-gateway's typed ServiceSpec. It is the single source both renderers read:
// the env carries NAMES and Secret references only (never a value — the JWT secret is a Vault/K8s
// Secret reference resolved at runtime), matching the composition root's EDEN_GATEWAY_* contract.
func Spec() servicespec.ServiceSpec {
	return servicespec.ServiceSpec{
		...
		Env: []servicespec.EnvVar{
			...
			// The JWT signing key is a Vault reference resolved at runtime; the manifest carries the
			// REFERENCE, never the value (the secrets no-leak contract).
			{Name: "EDEN_GATEWAY_JWT_SECRET_REF", Value: "vault://http-gateway/production#jwt-signing-key"},
		},
