#!/usr/bin/env bash
#
# libs/templates/go/http-gateway/ctl.sh — control script for the http-gateway application
# template (an OpenAPI-first, sqlc/pgx HTTP service that assembles the Eden Go libraries).
#
...
# per-template metadata and sources the shared library. project.json targets delegate here.
#
set -Eeuo pipefail
IFS=$'\n\t'
...
EDEN_TEMPLATE_NAME="http-gateway"
...
# shellcheck source=../../_ctl/template.sh
# shellcheck disable=SC1091
source "$PROJECT_ROOT/../../_ctl/template.sh"
