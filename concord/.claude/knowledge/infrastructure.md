...
- **`secrets/create-all.sh` silently does nothing (no namespaces processed)** — `cluster.yaml` failed to parse, so `yq -r '.namespaces[]'` returned empty and the namespace loop never ran. The usual culprit is an unquoted scalar containing `: ` (colon-space) in a `description:` or similar field, which `yq` rejects. Quote the value (e.g. `description: "Standalone CI platform (Helm chart: concord-ci)"`). Run `./infrastructure/ctl.sh office validate` to catch it.
