...
Refresh this file when: a new `NodeType` or `FixturePurpose` value is added, the live-lock derivation rules change, the MTIB-to-slot binding shape changes, the TestBedDesign profile schema is extended, or new slot fields are added.

## Entities

| Model | Role |
|---|---|
| `TestBedDesign` | The template — a versioned hardware design (PCB/wiring spec) bound 1:1 to a `TestPackage`. Concord stores the `profileTemplate` JSON (power config, GPIO pinout, slot defaults) but never interprets it; product-specific test code does. |
...
Schema: `prisma/schema.prisma:740-896` (TestBedDesign, Fixture, FixtureSlot, Node), `:126-130` (FixturePurpose), `:28-31` (NodeType).
...
A `TestBedDesign` is created when a test package upload includes a `fixture.yaml` manifest. The design's status mirrors the parent package's — denormalized so fixtures can join the design without joining the package.
...
| TestBedDesign upload (via TestPackage manifest) | `apps/backend/http-api/src/api/v2/products/test_packages*.py` |
...
- **TestBedDesign owns 1:1 of its TestPackage.** Enforced by `@unique` on `TestBedDesign.testPackageId`. A package either ships a fixture profile or it doesn't.
...
**`TestBedDesign` won't update** — the parent `TestPackage` is `RELEASED` (immutable). Upload a new test package version; the design is recreated from the new manifest.
