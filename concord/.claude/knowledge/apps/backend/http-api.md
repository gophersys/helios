...
|---|---|---|---|
| `POST /v2/fixture-claims` | Create. Body is XOR `fixtureId` vs `nodes[]` (`CreateClaimRequest` in `validators.py`). `ttlSeconds` defaults to 3600, clamped to 8h. Refuses 409 if the gate reports busy. After the row is written, **auto-provisions mtib-server** on every held node via `service.provision_mtibs_for_claim` (idempotent — returns existing deploy names on 409). Returns **502** + releases the claim if provisioning fails on any node. | `VALIDATION_RUN` for validation-typed targets, `MANUFACTURING_MANAGE` otherwise — classified by `service.is_validation_target`. | `fixture.claim.create` + `mtib.deployment.create` (when at least one deployment was created — reused ones are listed in details but don't generate a second audit row) |
...
| `POST /v2/fixture-claims/<id>/release` | Idempotent: ACTIVE → RELEASED, anything else passes through. **Tears down claim-owned mtib-server deployments** via `service.teardown_claim_mtibs` (label-matched — fixture-bound and standalone-node deployments survive). | Same as heartbeat. | `fixture.claim.release` with `reason="explicit"` + `mtib.deployment.delete` (when at least one deployment was deleted) |
...
- `service.provision_mtibs_for_claim(claim)` walks every held node (fixture slots or `claimedNodes`) and calls `services/kubernetes/mtib_deployments.create_mtib_deployment(..., claim_id=claim.id)` per node. Returns `{"created": [...], "reused": [...], "failed": [...]}` keyed by hostname. Idempotent — a node that already has a fixture-bound or standalone-node deployment is detected via `Node.metadata["deployment_name"]` and recorded as "reused" (no second pod, no claim-id label change). The mtib server template (`assets/templates/mtib_server_deployment.yaml`) carries a `corekinect.com/claim-id` label populated from this id — empty string for fixture-bound and standalone-node deployments. **After the create phase, the function blocks** until every held node answers a TCP probe on the MTIB gRPC port (default 50053) using `services/kubernetes/mtib_deployments.wait_for_mtibs_healthy`. The poll loop runs for at most `DEFAULT_PROVISIONING_WAIT_S` (60 s); nodes that remain unreachable past the deadline are moved into the `failed` list so the route layer releases the claim instead of handing the developer a lease against pods stuck in `ImagePullBackOff` / `CrashLoopBackOff`.
- `service._build_slot_bindings(claim)` additionally attaches a `podPhase` field per binding when the bound node's `metadata.deployment_name` points at a known mtib-server Deployment. The phase is read from `get_mtib_deployment_status(...)["pods"][0]["status"]` (`Running` / `Pending` / `ImagePullBackOff` / `CrashLoopBackOff` / `Unknown` / `null`). `corectl test status` colours this — green for `Running`, yellow for anything else — so a degraded slot is visible at a glance without a separate `kubectl get pod`.
- **Image digest pinning**: before submitting a Deployment, `create_mtib_deployment` HEADs the container registry's OCI manifest endpoint for the requested image and resolves the tag (`...:latest`, `...:v0.10.6`) to an immutable digest (`...@sha256:...`) which is written into the Deployment manifest. A 404 from the registry raises `MtibImageUnavailable` (code `MTIB_IMAGE_UNAVAILABLE`) BEFORE any K8s mutation, so the route layer returns 502 with that code instead of letting the cluster fall into `ImagePullBackOff`. Network errors (timeout, DNS) fall through with a warning and the bare tag — we don't want a flaky registry to block claim creation. The OCI request uses a 3 s timeout; already-digested refs and library-shorthand tags are passed through unchanged.
- **Claim-mode Deployment naming and Service**: `create_mtib_deployment(..., claim_id="...")` uses `mtib-claim-{claim_id[:8]}-{hostname}-s{slot}` so the name does not collide with the fixture-bound `mtib-{hostname}-s{slot}` Deployment on the same node. The claim-mode Deployment additionally has its `hostPort: 50053` stripped before submission (the fixture-bound deployment owns that port on the node) and a companion ClusterIP `Service` (same name, port 50053 → targetPort 50053) is created so runner pods can reach it inside the cluster via `<deploy_name>.<namespace>.svc.cluster.local:50053`. Teardown deletes both the Deployment and its Service via the same `corekinect.com/claim-id` label selector.
- `service.teardown_claim_mtibs(claim)` deletes only deployments whose `corekinect.com/claim-id` label matches `claim.id`. Implemented via `services/kubernetes/mtib_deployments.delete_mtib_deployments_for_claim(claim_id)` which does a k8s `list_namespaced_deployment` with `label_selector=corekinect.com/claim-id=<id>` then deletes each match. Fixture-bound deployments (`fixture-id` label set, `claim-id` empty) and standalone-node deployments (registered via `api/v2/nodes/nodes.register_node`, also `claim-id` empty) are **never** matched — the label-equality selector won't pair a non-empty needle to an empty value, so existing manufacturing/validation fixture deployments are immune.
- `service.sweep_expired_claims` is the background lifecycle driver — see the `claim-expiry-sweep` row in the scheduler table above. Called from `scheduling/scheduler._claim_expiry_sweep_loop` every 60 s. **Also calls `teardown_claim_mtibs` for each transition** so abandoned/expired claims don't leak pods.
- The reservation gate (`services/fixtures/reservation.is_fixture_busy` + `are_nodes_busy`) is the single source of truth for "is this hardware busy?". It unions ACTIVE `TestRun`, `ManufacturingSession`, and `FixtureClaim` rows — plus, for node-mode claims, the transitive case where a node is wired into a slot whose fixture has an active run/session OR an active fixture-mode claim. The fixture-mode-claim ↔ node-mode-claim cross-check is symmetric: `is_fixture_busy` looks at the inverse direction (node-mode claim holding one of the fixture's slot nodes), so the two modes can never overlap on the same hardware. Consulted by:
...
- **`ModuleNotFoundError: No module named 'protocols.mtib.mtib_pb2'` at startup.** The Dockerfile copies `libs/protocols/mtib/` into the image, but the gitignored `*_pb2.py` / `*_pb2_grpc.py` files are generated by `protocols:create`. Since v0.12.12 the http-api `containerize` target has `dependsOn: ["protocols:create"]`, so `nx update platform` will always run codegen first. If you still hit this, you likely bypassed Nx (e.g., `docker build` directly) — go through Nx. See [`../../libs/protocols.md`](../../libs/protocols.md) § Codegen flow.
