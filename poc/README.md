...
| `codeinsight/` | Codebase-insight analyzer: walks a git repo → one self-describing `Report` (churn, native-AST cyclomatic/cognitive/maintainability, hotspots, logical coupling, ownership/bus-factor, trends + a Views render-plan). Runs on real Eden `libs/` | Validates the `codeinsight` payload contract + the verified metric formulas (research note 06) before the gated `libs/go/codeinsight` build; surfaced the file-level-MI-saturation finding that shapes that build (forward-looking, Eden-named) |
