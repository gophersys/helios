...
function checkOwnership() {
  ...
  for (const owner of ownership.projects) {
    ...
    if (projects.has(owner.project)) throw new Error(`duplicate ownership: ${owner.project}`);
    ...
    declaredProjects.push({ ...owner, config: project });
  ...
  for (const parent of declaredProjects.filter(({ path }) => path !== ".")) {
    const children = declaredProjects.filter(({ path }) => path.startsWith(`${parent.path}/`));
    if (!children.length) continue;
    if (!parent.config.tags?.includes("kind:group")) throw new Error(`${parent.project}: nested projects require kind:group`);
    if (!parent.config.targets.check.dependsOn?.includes("^check")) throw new Error(`${parent.project}: check must depend on ^check`);
    const dependencies = new Set(parent.config.implicitDependencies ?? []);
    for (const child of children) {
      if (!dependencies.has(child.project)) throw new Error(`${parent.project}: missing child dependency ${child.project}`);
    }
  }
}
...
try {
  ...
  else throw new Error("usage: ./ctl.sh <check|sync|status|review>");
} catch (error) {
