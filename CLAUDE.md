...
- **Do the development work inside the devcontainer.** `.devcontainer/base` is the substrate for
  ...
  repository at `/workspace`, mounts the docker socket, and installs the pinned harnesses. Then
  run `ctl.sh exec -- <cmd>` or `ctl.sh shell`.
- The image contains the gate toolchain of ADR-0020: gofumpt, golangci-lint, govulncheck,
  ...
  the container.
- **hnslint comes from its own repository.** It is `gophersys/hnslint`, a public repository. The
  base Dockerfile installs it, pinned by `HNSLINT_VERSION`, so every image below `base` carries
  it. It is public because an image build cannot authenticate to a private repository, which is
  the same reason `gophersys/cictl` is public. Proven in the pod shape by
  `bash ctl.sh verify-runner-image e0c6bc5`, which asserts `hnslint on PATH`.
  This bullet once said the image held the tool while no image held it, and `gophersys/libs`
  failed `phase-gate implementation` with `missing required tool(s): hnslint`. This repository
  also held a second copy at `tools/hnslint`, and `post-create` built that copy into `GOPATH/bin`,
  which takes precedence on `PATH`. So a developer ran a working tree while CI ran the release.
  Both the copy and the build are deleted.
...
- **`hnslint`** is the structural check for HNS-1. Its source is the public repository
  `gophersys/hnslint`. This repository holds no copy of it. The base image installs it, pinned by
  `HNSLINT_VERSION` in `.devcontainer/base/Dockerfile`, so it is on `PATH` inside the container.
  To change the check, cut a release in `gophersys/hnslint` and raise the pin. Never build it from
  a working tree: that binary goes to `GOPATH/bin`, it takes precedence on `PATH`, and it hides
  the pinned one. The shared configuration of the linter is `libs/.golangci.yml`.
