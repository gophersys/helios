...
- **TestBed** — The Python class users subclass to declare a board revision's DUT-side wiring (MTIB-to-device pin map, J-Link mapping, UART routing, power rails). Lives in the user's test app at `testbeds/<board>/testbed.py`. The class is AST-extracted at upload time and used to populate Concord's `TestBedDesign` row — so a TestBed (user code) becomes a TestBedDesign (Concord storage). The rename from the older "Fixture" class name (2026-05) was to disambiguate from Concord's platform-level `Fixture` model.
