...
3. Picks the http-api pod, mints a JWT inside it using `JWT_SECRET_KEY` with `role: "ADMIN"` for the release-recorder user (`RELEASE_RECORDER_USER_ID` env var on the http-api Deployment; falls back to a `jared@corekinect.com` lookup — the project owner. Set `RELEASE_RECORDER_USER_ID` to avoid the person-specific fallback). The var is declared (empty) in each env's helm `config:` block (`values-{staging,production}.yaml`) and `docker-compose.yaml`; populate it with Jared's per-env `User.id` and redeploy to activate.
