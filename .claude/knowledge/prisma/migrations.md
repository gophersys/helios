...
- `20260518210000_add_test_package_framework` — adds `TestFramework` Postgres enum (`PYTEST | ZTEST`) and the `framework` column on `test_packages` with default `PYTEST`. Additive, safe to deploy without app changes — every existing row reads back as PYTEST and the runner dispatch (`runner_dispatch.py` + `manual.py`) treats that as the back-compat path (no override of container `command`). See `apps/backend/http-api.md` for the dispatch flow and `libs/python-corekinect.md` for the ztest runner.

- `20260521000000_add_fixture_claim` — adds `FixtureClaimStatus` Postgres enum (`ACTIVE | RELEASED | EXPIRED | ABANDONED`) and the `fixture_claims` + `claimed_nodes` tables that back the DEV_HOLD developer-lease feature. Fully additive — no existing table touched. Two binding modes share one lifecycle: fixture-mode (`fixtureId` set, whole fixture held) and node-mode (`claimed_nodes` populated, ad-hoc node set held). Indexed on `(fixtureId, status)` and `(userId, status)` for the reservation gate + per-user listing. FKs: `userId → users.id RESTRICT`, `fixtureId → fixtures.id RESTRICT` (claims block accidental fixture delete), `claimedNodes.claimId → fixture_claims.id CASCADE` (claim deletion drops its node bindings), `claimedNodes.nodeId → nodes.id RESTRICT`. See `prisma/schema-overview.md` cluster 5 for the model context and the `is_fixture_busy` reservation gate.
