...
// the typed resource store the route execute stages call. Its zero value is unusable; construct via
// New. Safe for concurrent use — a *pgxpool.Pool is concurrency-safe, and the store holds only the
// generated Querier over it.
...
func New(ctx context.Context, configuration Configuration, dependencies Dependencies) (*Persistence, error) {
	...
	return &Persistence{
		pool:          pool,
		resources:     &Resources{queries: generated.New(pool)},
		observability: dependencies.Observability,
	}, nil
}

// Resources returns the typed resource store the route execute stages call (accept-interfaces,
// return-concrete — the consumer receives a usable store, not another port). The store maps pgx's
// not-found sentinel to a typed errors.KindNotFound.
func (p *Persistence) Resources() *Resources { return p.resources }
